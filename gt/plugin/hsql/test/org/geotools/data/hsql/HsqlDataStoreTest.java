 ...
 */
public class HsqlDataStoreTest extends DataTestCase {
    HsqlDataStore data;
    ...
    protected void setUp() throws Exception {
        ...
        try {
            data = new HsqlDataStore(connFactory, null, getName());
        } catch (Exception e) {
            LOGGER.log(Level.INFO, "exception while making schema", e);
        }
        data.removeSchema(roadType);
        data.removeSchema(riverType);
    ...
    private void setUpSpecial() throws Exception {
    ...
    public void testFeatureEvents() throws Exception {
		...
		store1.removeFeatures(factory.createFidFilter("0"));
...
//		assertEquals( 0, listener2.events.size() );
    ...
    /**
     * This function is stolen from DefaultFeature equals method. We want to check for 
     * equality except for featureId which we expect to be different.
     *
...
//        assertTrue(reader instanceof JDBCFeatureReader);//FilteringFeatureReader);
    ...
    public void testGetFeatureReaderFilterTransaction()
        ...
        HsqlTransactionStateDiff state = (HsqlTransactionStateDiff) t.getState(data);
    ...
    public void testGetFeatureWriterRemove()
        ...
        //TODO: This is WAY too specific to the memoryDataStore
//      assertEquals(roadFeatures.length - 1,
//          data.features("road").size());
      //Instead, we'll get a new writer, and get a count of entries
      writer = data.getFeatureWriter("ROAD");
...
//            data.features("road").size());
    ...
    public void testGetFeaturesWriterModify()
        ...
        feature = null;
        FeatureReader reader = 
        	data.getFeatureReader( new DefaultQuery( "ROAD", rd1Filter ), 
        			Transaction.AUTO_COMMIT);
        if( reader.hasNext() )
        	feature = reader.next();
...
//        assertTrue(writer instanceof JDBCFeatureWriter);//FilteringFeatureWriter);
    ...
    public void testLockFeatureInteraction() throws IOException {
        ...
        assertFalse( isLocked( "ROAD", "0" ) );
        assertFalse( isLocked( "ROAD", "1" ) );
        assertFalse( isLocked( "ROAD", "2" ) );        
