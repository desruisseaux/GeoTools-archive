 ...
 * a format.
 * <p>
 * This format will only report itself to be &quot;available&quot;
 * if the JAI and JAI ImageI/O libraries are available.  Otherwise it
 * will be unavailable.  If a user attempts to create a new instance
 * of the format when the required libraries are unavailable, an
...
public class GeoTiffFormatFactorySpi implements GridFormatFactorySpi {
     ...
     * class if the required libraries are present.  If JAI and JAI Image
     * I/O are not present, will throw an
    ...
    /**
     * Informs the caller whether the libraries required by the GeoTiff
     * reader are installed or not.
