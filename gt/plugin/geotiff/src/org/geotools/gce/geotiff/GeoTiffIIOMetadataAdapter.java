 ...
 * the purpose of retrieving GeoTIFF metadata from an image.
 * 
 * <p>
 * All of the GeoKey values are included here as constants, and the portions of
 * the GeoTIFF specification pertaining to each have been copied for easy
 * access.
 * </p>
 * 
 * <p>
 * The majority of the possible GeoKey values and their meanings are NOT
 * reproduced here. Only the most important GeoKey code values have been
 * copied, for others see the specification.
 * </p>
 * 
 * <p>
...
public class GeoTiffIIOMetadataAdapter {
     ...
     * system used, and to which the raster space will be transformed:
     * unknown, Geocentric (rarely used), Geographic, Projected Coordinate
     * System, or user-defined. If the coordinate system is a PCS, then only
     * the PCS code need be specified. If the coordinate system does not fit
     * into one of the standard registered PCS'S, but it uses one of the
     * standard projections and datums, then its should be documented as a PCS
     * model with "user-defined" type, requiring the specification of
     * projection parameters, etc.  GeoKey requirements for User-Defined Model
     * Type (not advisable): GTCitationGeoKey
     */
    public static final int GTModelTypeGeoKey = 1024;

    /**
     * GTRasterTypeGeoKey  Key ID = 1025   Type =  Section 6.3.1.2 codes This
     * establishes the Raster Space coordinate system used; there are
    ...
    /**
     * GTCitationGeoKey  Key ID = 1026   Type = ASCII As with all the
     * "Citation" GeoKeys, this is provided to give an ASCII reference to
     * published documentation on the overall configuration of this GeoTIFF
     * file.
     */
    ...
    /**
     * GeographicTypeGeoKey  Key ID = 2048   Type = SHORT (code) Values =
     * Section 6.3.2.1 Codes
     * http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.3.2.1 This
     * key may be used to specify the code for the geographic coordinate
     * system used to map lat-long to a specific ellipsoid over the earth.
     * GeoKey Requirements for User-Defined geographic CS:  GeogCitationGeoKey
     ...
     * http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.3.2.2 This
     * key may be used to specify the horizontal datum, defining the size,
     * position and orientation of the reference ellipsoid used in
     * user-defined geographic coordinate systems.  GeoKey Requirements for
     ...
     * Meridians, the location of which is defined by its longitude relative
     * to Greenwich.
     */
    public static final int GeogPrimeMeridianLongGeoKey = 2061;

    /**
     * GeogLinearUnitsGeoKey  Key ID = 2052 Type = SHORT Values:  Section
     * 6.3.1.3 Codes
     * http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.3.1.3 Allows
    ...
    /**
     * GeogAngularUnitsGeoKey Key ID = 2054   Type = SHORT (code) Values =
     * Section 6.3.1.4  Codes  Allows the definition of geocentric CS Linear
     * units for user-defined GCS and for ellipsoids.  GeoKey Requirements for
     ...
     * 6.3.2.3 Codes
     * http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.3.2.3  This
     * key may be used to specify the coded ellipsoid used in the geodetic
     * datum of the Geographic Coordinate System.  GeoKey Requirements for
     * User-Defined Ellipsoid:  GeogCitationGeoKey [GeogSemiMajorAxisGeoKey,
     * [GeogSemiMinorAxisGeoKey | GeogInvFlatteningGeoKey] ]
     */
    public static final int GeogEllipsoidGeoKey = 2056;

    /**
     * GeogSemiMajorAxisGeoKey  Key ID = 2057 Type = DOUBLE Units: Geocentric
     * CS Linear Units Allows the specification of user-defined Ellipsoid
     * Semi-Major Axis (a).
     */
    public static final int GeogSemiMajorAxisGeoKey = 2057;

    /**
     * GeogSemiMinorAxisGeoKey  Key ID = 2058 Type = DOUBLE Units: Geocentric
     * CS Linear Units Allows the specification of user-defined Ellipsoid
     * Semi-Minor Axis (b).
     */
     ...
     * GeogSemiMinorAxisGeoKey instead, and set it equal to the semi-major
     * axis length.
     */
    public static final int GeogInvFlatteningGeoKey = 2059;

    /**
     * GeogAzimuthUnitsGeoKey Key ID = 2060   Type = SHORT (code) Values =
     * Section 6.3.1.4 Codes  This key may be used to specify the angular
     * units of measurement used to defining azimuths, in geographic
     * coordinate systems. These may be used for defining azimuthal parameters
     * for some projection algorithms, and may not necessarily be the same
    ...
    /**
     * ProjectedCSTypeGeoKey  Key ID = 3072 Type = SHORT (codes) Values:
     * Section 6.3.3.1 codes This code is provided to specify the projected
     * coordinate system.  GeoKey requirements for "user-defined" PCS
     * families: PCSCitationGeoKey ProjectionGeoKey
     */
     ...
     * documentation on the Projected Coordinate System particularly if this
     * is a "user-defined" PCS.
     */
     ...
     * specification of the coordinate transformation method and projection
     * zone parameters. Note : when associated with an appropriate Geographic
     ...
     * "user-defined" Coordinate Transformations: PCSCitationGeoKey
    ...
    /**
     * ProjStdParallel1GeoKey Key ID = 3078  Type = DOUBLE Units:
     * GeogAngularUnit  Alias: ProjStdParallelGeoKey (from Rev 0.2) Latitude
     * of primary Standard Parallel.
     */
    public static final int ProjStdParallel1GeoKey = 3078;

    /**
     * ProjStdParallel2GeoKey Key ID = 3079 Type = DOUBLE Units:
     * GeogAngularUnit  Latitude of second Standard Parallel.
     */
    public static final int ProjStdParallel2GeoKey = 3079;

    /**
     * ProjNatOriginLongGeoKey Key ID = 3080 Type = DOUBLE Units:
     * GeogAngularUnit  Alias: ProjOriginLongGeoKey Longitude of
     * map-projection Natural origin.
     */
    public static final int ProjNatOriginLongGeoKey = 3080;

    /**
     * ProjNatOriginLatGeoKey Key ID = 3081 Type = DOUBLE Units:
     * GeogAngularUnit Alias: ProjOriginLatGeoKey Latitude of map-projection
     * Natural origin.
     */
    ...
    /**
     * ProjFalseNorthingGeoKey Key ID = 3083 Type = DOUBLE Units:
     * ProjLinearUnit  Gives the northing coordinate of the map projection
     * Natural origin.
     */
     ...
     * ProjLinearUnit  Gives the easting coordinate of the false origin. This
     * is NOT the False Easting, which is the easting attached to the Natural
     * origin.
     */
     ...
     * ProjLinearUnit  Gives the northing coordinate of the False origin. This
     * is NOT the False Northing, which is the northing attached to the
     * Natural origin.
     */
     ...
     * Longitude of Center of Projection. Note that this is not necessarily
    ...
    /**
     * ProjCenterEastingGeoKey Key ID = 3090 Type = DOUBLE Units:
     * ProjLinearUnit  Gives the easting coordinate of the center. This is NOT
     * the False Easting.
     */
     ...
     * ProjLinearUnit  Gives the northing coordinate of the center. This is
     * NOT the False Northing. NOTE this value is incorrectly named at
     * http://www.remotesensing.org/geotiff/spec/geotiff2.7.html#2.7
     */
    public static final int ProjCenterNorthingGeoKey = 3091;

    /**
     * ProjScaleAtNatOriginGeoKey Key ID = 3092 Type = DOUBLE Units: none
     * Alias: ProjScaleAtOriginGeoKey (Rev. 0.2) Scale at Natural Origin. This
     * is a ratio, so no units are required.
     */
    public static final int ProjScaleAtNatOriginGeoKey = 3092;

    /**
     * ProjScaleAtCenterGeoKey Key ID = 3093  Type = DOUBLE Units: none Scale
     * at Center. This is a ratio, so no units are required.
     */
    public static final int ProjScaleAtCenterGeoKey = 3093;

    /**
     * ProjAzimuthAngleGeoKey Key ID = 3094 Type = DOUBLE Units:
     * GeogAzimuthUnit Azimuth angle east of true north of the central line
     * passing through the projection center (for elliptical (Hotine) Oblique
     * Mercator). Note that this is the standard method of measuring azimuth,
     * but is opposite the usual mathematical convention of positive
    ...
    /**
     * VerticalCSTypeGeoKey Key ID = 4096   Type = SHORT (code) Values =
     * Section 6.3.4.1  Codes
     * http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.3.4.1 This
     * key may be used to specify the vertical coordinate system.
     */
    public static final int VerticalCSTypeGeoKey = 4096;

    /**
     * VerticalCitationGeoKey Key ID = 4097 Type = ASCII Values =  text  This
     * key may be used to document the vertical coordinate system used, and
     * its parameters.
     */
    public static final int VerticalCitationGeoKey = 4097;

    /**
     * VerticalDatumGeoKey Key ID = 4098    Type = SHORT (code) Values =
     * Section 6.3.4.2  codes
     * http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.3.4.2 This
     * key may be used to specify the vertical datum for the vertical
     * coordinate system.
     */
    public static final int VerticalDatumGeoKey = 4098;

    /**
     * VerticalUnitsGeoKey Key ID = 4099    Type = SHORT (code) Values =
     * Section 6.3.1.3  Codes
     * http://www.remotesensing.org/geotiff/spec/geotiff6.html#6.3.1.3 This
     * key may be used to specify the vertical units of measurement used in
     * the geographic coordinate system, in cases where geographic CS's need
    ...
    /**
     * 6.3.1.1 Model Type Codes Ranges: 0              = undefined [   1,
     * 32766] = GeoTIFF Reserved Codes 32767          = user-defined [32768,
     ...
     * angular units, units of time (e.g. for radar-return), CCD-voltages,
     * etc. For this reason there will be a single, unique range for each kind
     * of unit, broken down into the following currently defined ranges:
     * Ranges: 0             = undefined [   1,  2000] = Obsolete GeoTIFF
     * codes [2001,  8999] = Reserved by GeoTIFF [9000,  9099] = EPSG Linear
     * Units. [9100,  9199] = EPSG Angular Units. 32767         = user-defined
     * unit [32768, 65535]= Private User Implementations
     */
     ...
     * consists of both a datum and a Prime Meridian. Some of the names are
     * very similar, and differ only in the Prime Meridian, so be sure to use
     * the correct one. The codes beginning with GCSE_xxx are unspecified GCS
     * which use ellipsoid (xxx); it is recommended that only the codes
     * beginning with GCS_ be used if possible.  Ranges: 0 = undefined [    1,
     * 1000] = Obsolete EPSG/POSC Geographic Codes [ 1001,  3999] = Reserved
     * by GeoTIFF [ 4000, 4199]  = EPSG GCS Based on Ellipsoid only [ 4200,
     * 4999]  = EPSG GCS Based on EPSG Datum [ 5000, 32766] = Reserved by
     * GeoTIFF 32767          = user-defined GCS [32768, 65535] = Private User
     * Implementations Values: Note: Geodetic datum using Greenwich PM have
     * codes equal to the corresponding Datum code - 2000. Ellipsoid-Only GCS:
     * Note: the numeric code is equal to the code of the correspoding EPSG
     * ellipsoid, minus 3000. Note: Only a handful of values have been
     * reproduced here, for the remainder see the GeoTIFF specification.
     */
    ...
    /**
     * 6.3.3.1 Projected CS Type Codes Ranges: [    1,   1000]  = Obsolete
     * EPSG/POSC Projection System Codes [20000,  32760]  = EPSG Projection
     * System codes 32767            = user-defined [32768,  65535]  = Private
     * User Implementations Special Ranges: 1. For PCS utilizing GeogCS with
     * code in range 4201 through 4321: As far as is possible the PCS code
     * will be of the format gggzz where ggg is  (geodetic datum code -4000)
     * and zz is zone. 2. For PCS utilizing GeogCS with code out of range 4201
     * through 4321  (i.e. geodetic datum code 6201 through 6319). PCS code
     * 20xxx where  xxx is a sequential number. 3. Other: WGS72 / UTM northern
     * hemisphere: 322zz where zz is UTM zone number WGS72 / UTM southern
     * hemisphere: 323zz where zz is UTM zone number WGS72BE / UTM northern
     * hemisphere: 324zz where zz is UTM zone number WGS72BE / UTM southern
     * hemisphere: 325zz where zz is UTM zone number WGS84 / UTM northern
     * hemisphere: 326zz where zz is UTM zone number WGS84 / UTM southern
     * hemisphere: 327zz where zz is UTM zone number US State Plane (NAD27):
     * 267xx/320xx  US State Plane (NAD83):  269xx/321xx Note: These are only
    ...
    /**
     * The DOM element ID (tag) for a single TIFF double. The value is stored
     ...
     * &quotquiet&quot in the sense that it should not throw any exceptions
     * but only return null in the event that the data organization is not as
     * expected.
     *
                    ...
                    if (sequence != null) {
                        if (sequence.getNodeName().equals(TIFF_ASCIIS_TAG)) {
                            ...
                            result = getTiffAscii((IIOMetadataNode) sequence,
                                    rec.getValueOffset(), rec.getCount());
                        } else {
                            // value is numeric
     ...
     * For more information see the GeoTIFF specification.
     *
    ...
    public GeoKeyRecord getGeoKeyRecord(int keyID) {
        ...
        IIOMetadataNode tiffShortsNode = (IIOMetadataNode) geoKeyDir
            .getFirstChild();
    ...
    public GeoKeyRecord getGeoKeyRecordByIndex(int index) {
        ...
        Node n = keys.item(index);
        Node locNode = keys.item(index + 1);
        Node countNode = keys.item(index + 2);
        Node offsetNode = keys.item(index + 3);
    ...
    /**
     * Gets an array of int values stored in a TIFFShorts element that contains
     * a sequence of TIFFShort values.
     *
    ...
    private int[] getTiffShorts(IIOMetadataNode tiffField) {
        IIOMetadataNode shortsElement = (IIOMetadataNode) tiffField
            .getFirstChild();
    ...
    private double[] getTiffDoubles(IIOMetadataNode tiffField) {
        IIOMetadataNode doublesElement = (IIOMetadataNode) tiffField
            .getFirstChild();
    ...
    /**
     * Gets a single double value at the specified index from a sequence of
     * TIFFDoubles
     *
     * @param tiffField An IIOMetadataNode pointing to a TIFFField element that
     *        contains a TIFFDoubles element.
    ...
    private String getTiffAscii(IIOMetadataNode tiffField, int start, int length) {
        IIOMetadataNode asciisElement = (IIOMetadataNode) tiffField
            .getFirstChild();
     ...
     * contained in the offset. Otherwise, there is one or more value in the
     * specified external Tiff tag. The number is specified by the count
     * field, and the offset into the record is the offset field.
     * </p>
     */
