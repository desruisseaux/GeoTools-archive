...
public class ArcSDEFeatureWriter implements FeatureWriter {
    ...
    private static final Logger LOGGER = Logger.getLogger(
            ArcSDEFeatureWriter.class.getPackage().getName());
    ...
    private Integer[] attributeIndexes;
     ...
     * @param features
     */
    public ArcSDEFeatureWriter(ArcSDEDataStore store, ArcTransactionState state, SeLayer layer, List features) 
    {
        if (features != null) 
        {
     ...
     */
    public ArcSDEFeatureWriter(ArcSDEDataStore store, ArcTransactionState state, SeLayer layer) 
    {
        this(store, state, layer, null);
    }

    /**
     * Provides the <code>FeatureType</code> that is acceptable for features handled
     * by this <code>FeatureWriter</code>
     *
     * @return DOCUMENT ME!
     */
    ...
    /**
     * Implements an operation similar to next in java.util.Iterator.  This allows the 
     * caller to iterate over features obtained from the backing store and to instantiate
     * new features and add them to the layer in the backing store.
     *
     * @return If no 'next feature' is available then a new feature object is created
     * and returned to the caller so that they may modify its contents and later write
    ...
    public void remove() throws IOException 
    {
        ...
        else 
        {
            ...
            try 
            {
                ...
                seDelete.fromTable(layer.getQualifiedName(), "FID = '" + feature.getID() + '\'');
            } 
            catch (Exception e) 
            {
                LOGGER.log(Level.WARNING, e.getMessage(), e);
     ...
     * feature to the backing store because it either does not exist or the 
     * backing store throws its own exception that indicates an error.
     */
    public void write() throws IOException 
    {
        ...
        try 
        {
            ...
            if (notInserted) 
            {
                ...
                {
                    setRowValue(row, i, feature.getAttribute(attributeIndexes[i].intValue()));
                }
            ...
            else 
            {
                ...
                String[] cols = getColumns(attributeTypes, connection);
                String featureId = feature.getID().substring(feature.getID().lastIndexOf('.') + 1, feature.getID().length());
                update.toTable(layer.getQualifiedName(), cols, "SHAPE = " + featureId);
                ...
                for (int i = 0; i < cols.length; i++) 
                {
                    Object value = feature.getAttribute(attributeIndexes[i].intValue());
    ...
    /**
     * Indicates whether or not this <code>FeatureWriter</code> contains
     * more feature instances or not.
     *
    ...
    {
        try {
        return (features != null && features.size() > 0 && currentIndex + 1 < features.size());
        } catch (Throwable t) {
            t.printStackTrace();
            throw new IOException(t.getMessage());
        }
    ...
    private synchronized String[] getColumns(AttributeType[] attributeTypes, SeConnection connection) 
        throws SeException
    {
        if (columnDefinitions == null) 
        {
            ...
            attributeIndexes = new Integer[indexes.size()];
            columnList.toArray(columns);
            indexes.toArray(attributeIndexes);
        }
        return columns;
    }
			...
			case SeColumnDefinition.TYPE_SHAPE: 
            {
				if (value != null) 
                {
                    try 
                    {
                        ...
                        SeShape shape = geometryBuilder.constructShape((Geometry)value, layer.getCoordRef());
