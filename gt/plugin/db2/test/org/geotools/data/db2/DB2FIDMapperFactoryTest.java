...
public class DB2FIDMapperFactoryTest extends DB2TestCase {
    public void testMappers() throws Exception {
        ...
        assertEquals(tableName, wrapperDesc, "class org.geotools.data.jdbc.fidmapper.AutoIncrementFIDMapper:1:IDCOL:4:-1:-1:false:true:");
        
        conn = getLocalConnection();
       	tableName = "FIDCHARPRIKEY";
        fm = fmFact.getMapper(catalog, schema, tableName, conn);   
        wrapperDesc = toString(fm);
        assertEquals(tableName, wrapperDesc, "class org.geotools.data.jdbc.fidmapper.BasicFIDMapper:1:IDCOL:12:32:32:true:false:");
        
        conn = getLocalConnection();
       	tableName = "FIDNOPRIKEY";
        fm = fmFact.getMapper(catalog, schema, tableName, conn);   
        wrapperDesc = toString(fm);
        assertEquals(tableName, wrapperDesc, "class org.geotools.data.db2.DB2NullFIDMapper:0::false:false:");
