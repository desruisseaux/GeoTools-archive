...
public class SQLEncoderOracle extends SQLEncoder {
	...
	private static final Logger LOGGER = Logger
			.getLogger("org.geotools.filter.SQLEncoderOracle");
	...
	private void doBBoxFilter(GeometryFilter geomFilter) throws IOException {
		...
		if ((left != null) && (right != null)) {
			...
			LOGGER
					.warning("Invalid filter. Cannot have a Geometry filter with only one expression.");
		}
	}

	private void doSdoRelate(GeometryFilter geomFilter) throws IOException {
		String mask = (String) SDO_RELATE_MASK_MAP.get(new Short(geomFilter
				.getFilterType()));
		Expression left = geomFilter.getLeftGeometry();
		Expression right = geomFilter.getRightGeometry();
	...
	private String toSDOGeom(Polygon polygon, int srid) {
		...
		if (polygon.getExteriorRing() != null) {
			...
			Coordinate[] coordinates = polygon.getExteriorRing()
					.getCoordinates();
			...
			LOGGER
					.warning("No Exterior ring on polygon.  "
	...
	/**
	 * Handles Geometry Filter encoding. Currently only supports the encoding of
	 * GEOMETRY_BBOX filters. If a GEOMETRY_BBOX filter is encounter it will be
	 * converted into an SDO_RELATE() function. If another filter is found,
	 * nothing will happen.
	 * 
	 * @param geomFilter
	 *            The geometry filter to encode.
	 * 
	...
	public void visit(GeometryFilter geomFilter) {
		...
		try {
			...
			} else if (SDO_RELATE_MASK_MAP.get(new Short(geomFilter
					.getFilterType())) != null) {
	 ...
	 * 
	 * @param literal
	 *            The Literal expression to encode.
	 * 
