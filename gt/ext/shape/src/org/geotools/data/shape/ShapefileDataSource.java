 ...
 * </ol>
 * 
 * @deprecated Use org.geotools.data.shape.ShapefileDataStore
 * @version $Id$
...
public class ShapefileDataSource extends AbstractDataSource {
  ...
  public void getFeatures(FeatureCollection collection,final Query query) throws DataSourceException {
    try {
      ...
      // Create the FeatureType based on the dbf and shape
  ...
  public FeatureType getSchema() throws DataSourceException{
    try {
      ...
      throw new DataSourceException("Invalid shape",e);
    } catch (IOException e) {
      throw new DataSourceException("IO problem reading shape",e);
    }
  ...
  throws DataSourceException {
    ...
    try {
      ...
      throw new DataSourceException("Something went wrong during shape saving", se);
    } catch (DbaseFileException dfe) {
      throw new DataSourceException("Something went wrong during shape saving", dfe);
    } catch(IOException e) {
      throw new DataSourceException("IOException during shape saving", e);
    } catch(RuntimeException e) {
      throw new DataSourceException("Something went wrong during shape saving", e);
    }
  }
  ...
  private void writeDbf(FeatureCollection featureCollection) throws DbaseFileException,IOException {
    // welcome to the nastiest code in shape...
    ...
    for(int i = 0; i < types.length; i++) {
      ...
      Date.class.isAssignableFrom(currType)) {
        ...
        "shape: unsupported type found in feature schema : " +
        currType.getName()
        );
      }
    }
    ...
    for(int i = 0; i < types.length; i++) {
      ...
      if((colType == Integer.class) || (colType == Short.class) || (colType == Byte.class)) {
        ...
        if(maxlength > 255) {
          throw new DbaseFileException(
          "shape does not support strings longer than 255 characters");
        }
  ...
  public GeometryCollection makeShapeGeometryCollection(FeatureCollection fc) throws DataSourceException {
    ...
    if (type == ShapeType.NULL) {
      ...
      "Could not determine shape type - data is either all GeometryCollections or empty");
    }
