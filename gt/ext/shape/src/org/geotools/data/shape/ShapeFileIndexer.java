...
import org.geotools.data.shape.shp.ShapefileHeader;
...
public class ShapeFileIndexer {
    ...
    private static final Logger LOGGER =
        Logger.getLogger("org.geotools.data.shapefile");
    
    /**
     * Minimum time a thread will wait for index build by another thread
     * by default this is 2 seconds 
     */
    private static final long MIN_WAIT_TIME = 2000;
    
    /**
     * Maximum time a thread will wait for index build by another thread
     * by default this is 5 minutes (is it too much?) 
     */
     ...
     * case <b>zero</b> is reurned as result of the indexing process.
     * 
    ...
    throws MalformedURLException, IOException,
           TreeException, StoreException, LockTimeoutException
    {
        ...
        try {
            if (!RTREE.equals(this.idxType) && !QUADTREE.equals(this.idxType)) {
                throw new TreeException("Index type must be " + RTREE +
                                        " or " + QUADTREE);
            }
            ...
            ShapefileReader reader = new ShapefileReader(channel, false, true);
            ...
            if (!treeFile.renameTo(finalFile)) {
                throw new TreeException("Unable to rename " + treeFile +
                                         " to " + finalFile +
                                         " cannot commit the new index!");
            }
     ...
     * @throws IOException
     */
    private int buildRTree(ShapefileReader reader, 
                           File rtreeFile, 
                           boolean verbose) 
    throws TreeException, LockTimeoutException, IOException 
    {
        ...
        while (reader.hasNext()) {
            ...
            rtree.insert(new Envelope(record.minX,
                                      record.maxX,
                                      record.minY,
                                      record.maxY),
                         data);
    ...
    private int buildQuadTree(ShapefileReader reader, 
                              File file,
                              boolean verbose)
    throws IOException, StoreException
    {
        ...
        Envelope bounds = new Envelope(header.minX(), 
                                       header.maxX(), 
                                       header.minY(), 
                                       header.maxY());
        ...
        while (reader.hasNext()) {
            ...
            tree.insert(cnt++, new Envelope(rec.minX, rec.maxX, 
                                            rec.minY, rec.maxY));
