...
public class FileSystemPageStore extends PageStore {
    ...
    throws TreeException
    {
        ...
        if (file.exists() && file.length() != 0) {
            throw new TreeException("Cannot set dataDefinition, " +
                                     "maxNodesEntries and "+
                                     "minNodeEntries to the existing index " +
                                     file);
        }
        if (file.isDirectory()) {
            throw new TreeException("Cannot use a directory as index!");
	...
	/**
	 * @see org.geotools.index.rtree.PageStore#getNode(long, org.geotools.index.rtree.Node)
	 */
	...
	public Entry createEntryPointingNode(Node node) {
        ...
        return new Entry(new Envelope(fn.getBounds()),
                         new Long(fn.getOffset()));
	}
