...
public class RTree {
    ...
    throws TreeException, UnsupportedFilterException 
    {
        ...
        return env.contains(root.getBounds()) ?
                root.getBounds() : 
    ...
    */
     ...
     * @throws TreeException
     */
    private Envelope getBoundsInternal(final Envelope query,
                                       final Node node)
    throws TreeException
    {
     ...
     * @throws TreeException
     */
    private void searchNode(final Envelope query,
                            final Node node,
    ...
    private Node chooseLeaf(Node node, Entry newEntry) throws TreeException {
        ...
        Entry best = null;
        Envelope env = null;
        double lastArea = Double.POSITIVE_INFINITY, currentArea = 0d;
        double w = 0, h = 0, nw = 0, nh = 0;
        ...
        for (Iterator iter = entries.iterator(); iter.hasNext(); ) {
            ...
            if (currentArea < lastArea) {
                ...
                best = element;
                        ...
                        (this.getEntryArea(best) >
                         this.getEntryArea(element)))
            {
                best = element;
            }
        }
        ...
        return this.chooseLeaf(this.store.getNode(best, node),
                               newEntry);
    }
    ...
    private Node[] splitNode(Node node) throws TreeException {
        ...
        while (true) {
            if (entries.size() == 0) {
                 ...
                 * to the group
                 */
                if (ret[0].getEntriesCount() + entries.size() <=
                    this.store.getMinNodeEntries())
                {
                ...
                if (ret[1].getEntriesCount() + entries.size() <=
                    this.store.getMinNodeEntries())
                {
                    for (int i = 0; i < entries.size(); i++) {
    ...
    private Entry[] quadraticPickSeeds(Entry[] entries) {
        ...
        Envelope env = null;
        double actualD = 0d, choosedD = Double.NEGATIVE_INFINITY;
    ...
    private Entry quadraticPickNext(Node[] nodes, ArrayList entries) {
        ...
        double diff = 0d, maxDiff = Double.NEGATIVE_INFINITY;
        Envelope e = null;        
        for (int i = 0; i < entries.size(); i++) {
    ...
    /**
     * Deletes the entry with the specified <code>Envelope</code>
     * as its bounds.<br>
        ...
        try {
            ...
            if (node == null) {
                throw new TreeException(
                    "No node found with the supplied envelope: " +
                    env);
            }
     ...
     * @param entries A <code>Collection</code> used to store
     *        Node Entry
     */
    ...
    /**
     * Finds the first leaf node that contains the element with
    ...
    private double getAreaDifference(Envelope env, Entry e1, Entry e2) {
        return this.getEnvelopeArea(env) -
               this.getEntryArea(e1) -
    ...
    private String dump(Node node, int indent) throws TreeException {
        ...
        spc.append("  ");
        for (int i = 0; i < node.getEntriesCount(); i++) {
            ret.append(spc).append(node.getEntry(i)).append(System.getProperty("line.separator"));
            if (!node.isLeaf()) {
