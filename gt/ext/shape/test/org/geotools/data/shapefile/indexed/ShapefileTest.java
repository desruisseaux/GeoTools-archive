...
public class ShapefileTest extends TestCaseSupport {
  ...
  public void testIndexFile() throws Exception {
    ShapefileReader reader1 = new ShapefileReader(getTestResourceChannel(STATEPOP), lock);
    ShapefileReader reader2 = new ShapefileReader(getReadableFileChannel(STATEPOP), lock);
  ...
  public void testSkippingRecords() throws Exception {
    ShapefileReader r = new ShapefileReader(getTestResourceChannel(STATEPOP), lock);
    int idx = 0;
    while (r.hasNext()) {
        idx++;
        r.nextRecord();
  ...
  public void testShapefileReaderRecord() throws Exception {
    ShapefileReader reader = new ShapefileReader(getTestResourceChannel(STATEPOP), lock);
  ...
  private void loadShapes(String resource,int expected) throws Exception {
    ShapefileReader reader = new ShapefileReader(getTestResourceChannel(resource), lock);
    int cnt = 0;
    while (reader.hasNext()) {
  ...
  private void loadMemoryMapped(String resource,int expected) throws Exception {
    ShapefileReader reader = new ShapefileReader(getReadableFileChannel(resource), lock);
    int cnt = 0;
    while (reader.hasNext()) {
      reader.nextRecord().shape();
      cnt++;
