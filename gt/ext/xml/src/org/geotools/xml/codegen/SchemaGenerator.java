...
public class SchemaGenerator extends AbstractGenerator {
	...
	public void addTypeMapping(
		String namespace, String name ,AttributeType gtType
	) {
		if (namespace == null) {
	...
	public void generate() throws Exception {
		...
		for (Iterator itr = types.values().iterator(); itr.hasNext();) {
			...
			gtSchema.put( 
				new org.geotools.util.Name( schema.getTargetNamespace(), gtType.getName() ), 
				gtType
			);
		}
...
//	}
	...
	private AttributeType createType(/*XSDSimpleTypeDefinition*/XSDTypeDefinition xsdType) {
...
//		);
		...
		AttributeType gtType = 
			AttributeTypeFactory.newAttributeType( xsdType.getName(), Object.class );
		types.put(xsdType, gtType);
		return gtType;
	}
...
//	}
	...
	private Name name(XSDTypeDefinition type) {
		return new org.geotools.util.Name(type.getTargetNamespace(),type.getName());
	}
