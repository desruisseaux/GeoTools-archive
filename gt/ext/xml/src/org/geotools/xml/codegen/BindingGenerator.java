...
public class BindingGenerator extends AbstractGenerator {
	...
	public void generate( XSDSchema schema ) {
		...
		if ( generateAttributes ) {
			...
			for ( Iterator a = attributes.iterator(); a.hasNext(); ) {
				XSDAttributeDeclaration attribute = 
					(XSDAttributeDeclaration) a.next();
		...
		if ( generatingBindingConfiguration ) {
			try {
				String result = 
					execute( "BindingConfigurationTemplate", new Object[]  { schema, components } ); 
				write( result, prefix( schema ).toUpperCase() + "BindingConfiguration" );
			} 
			catch (Exception e) {
