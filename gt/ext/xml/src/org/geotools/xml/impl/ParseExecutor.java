...
public class ParseExecutor implements Visitor {
	...
	public ParseExecutor(
		InstanceComponent instance,Node node,MutablePicoContainer context
	) {
	...
	public void visit(Binding binding) {
		...
		binding = (Binding) 
			context.getComponentInstanceOfType(binding.getClass());
		...
		try {
			...
			if (binding instanceof SimpleBinding) {
				...
				result = ((ComplexBinding)binding)
					.parse((ElementInstance)instance,node,result);
			}
				...
				for (Iterator itr = facets.iterator(); itr.hasNext();) {
					...
					if ("whiteSpace".equals(facet.getFacetName())) {
						Whitespace whitespace = 
							Whitespace.valueOf(facet.getLexicalValue());
						if (whitespace != null) {
