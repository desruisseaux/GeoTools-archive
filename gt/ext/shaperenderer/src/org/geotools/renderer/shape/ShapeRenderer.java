...
public class ShapeRenderer {
    static final Logger LOGGER = Logger.getLogger("org.geotools.renderer.shape");
...
//      graphics.setTransform(at);
        ...
        for( int i = 0; i < layers.length; i++ ) {
            ...
            try {
                ...
                // and process them
                processStylers(graphics, currLayer, currLayer.getStyle().getFeatureTypeStyles(), transform,
    ...
    private void processStylers( Graphics2D graphics, MapLayer currLayer, FeatureTypeStyle[] featureStylers, AffineTransform at, CoordinateReferenceSystem destinationCrs ) {
        if (LOGGER.isLoggable(Level.FINE)) {
        ...
        for( int i = 0; i < featureStylers.length; i++ ) {
            ...
            ReadableByteChannel channel=ds.getReadChannel(ds.shpURL);
            ShapefileReader shpreader=new ShapefileReader(channel);
