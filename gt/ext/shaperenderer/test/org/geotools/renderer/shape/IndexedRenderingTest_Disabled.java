...
public class IndexedRenderingTest_Disabled extends TestCase {
	...
	protected void tearDown() {
		try {
			TestData.file(IndexedRenderingTest_Disabled.class, "lakes.qix").delete();
		} catch (IOException e) {
		}
		try {
			TestData.file(IndexedRenderingTest_Disabled.class, "lakes.grx").delete();
		} catch (IOException e) {
		}
		try {
			TestData.file(IndexedRenderingTest_Disabled.class, "streams.qix").delete();
		} catch (IOException e) {
		}
		try {
			TestData.file(IndexedRenderingTest_Disabled.class, "streams.grx").delete();
		} catch (IOException e) {
		}
	}
	...
	public void testQuadTree() throws Exception{
		...
		performRenderTest(IndexInfo.QUAD_TREE, 
        		...
        		new Envelope(553330.3289999997,570423.7210000004,5233544.282249999,5245830.157749998),
        		91);
		...
		performRenderTest(IndexInfo.QUAD_TREE, 
				...
				new Envelope( 552383.9726608695,569483.1330782612,5233875.318546738,5246165.340096739), 
				97);
		
		
	}

	private void performRenderTest(byte treeType, FeatureSource ds, Style s, int expectedFeatures) throws IOException, IllegalFilterException, Exception {
	 ...
	 * @throws Exception
	 */
	private void performRenderTest(byte treeType, FeatureSource ds, Style s, Envelope env, int expectedFeatures) throws IOException, IllegalFilterException, Exception {
		MapContext map = new DefaultMapContext();
        map.addLayer(ds,s);
        ShapefileRenderer renderer = new ShapefileRenderer(map);
        assertEquals(treeType, renderer.layerIndexInfo[0].treeType );
        map.setAreaOfInterest(map.getLayerBounds(), ds.getSchema().getDefaultGeometry().getCoordinateSystem());
        if( env==null ){
        ...
        TestUtilites.showRender("testQuadTree", renderer, 1000, env, expectedFeatures);
        
	}
	
	public void testRTree() throws Exception{
		String file=TestData.file(IndexedRenderingTest_Disabled.class, "lakes.shp").toString();
		ShapeFileIndexer indexer=new ShapeFileIndexer();
		...
		performRenderTest(IndexInfo.R_TREE, 
				TestUtilites.getDataStore("lakes.shp").getFeatureSource(),
        		TestUtilites.createTestStyle("lakes", null),
        		new Envelope(552238.3401249995,570174.7868350003,5233300.242658593,5246192.063731406),
        		93);
