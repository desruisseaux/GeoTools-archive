...
public class ArcSDERasterOneBitPerBandProducerImpl extends ArcSDERasterProducer {
	...
	/**
	 * Use this constructor if you need to pass a particular implementation to code which will later inject the SeRasterAttr, maskType and source BufferedImage later.
	 */
	public ArcSDERasterOneBitPerBandProducerImpl() {
		super(null, null, SeRasterScanLineGenerator.MASK_ALL_ON);
	}

	public ArcSDERasterOneBitPerBandProducerImpl(SeRasterAttr attr, BufferedImage sourceImage, int maskType) {
	...
	public void setSourceImage(BufferedImage sourceImage) {
		for (int i = 0; i < sourceImage.getSampleModel().getSampleSize().length; i++) {
			if (sourceImage.getSampleModel().getSampleSize(i) != 1) {
				throw new IllegalArgumentException("Source image must be one bit per sample on all bands.  Band " + i + " has " + sourceImage.getSampleModel().getSampleSize(i) + " bits per sample.");
			}
		}
		if (!(sourceImage.getData().getDataBuffer() instanceof DataBufferByte)) {
			throw new IllegalArgumentException("Currently one-bit-per-band raster loading is only supported from images backed with a DataBufferByte");
		}
	 ...
	 * 
	 * Also take a look at {@link SeRasterScanLineProducer} for an (opaque) ESRI supplied implementation of this functionality that only works with some sort of 8-bit images.
	 * 
	 * Note that due to some synchronization problems inherent in the SDE api code, the startProduction() method MUST return before consumer.setScanLines() or consumer.setRasterTiles() is called. Hence the thread implementation.
	 * 
	 */
