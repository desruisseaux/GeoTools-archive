...
public class TestData {
	...
	private static final Logger LOGGER = org.geotools.util.logging.Logging
			.getLogger(TestData.class.getPackage().getName());

	public static final String[] TEST_TABLE_COLS = { "INT32_COL", "INT16_COL",
			"FLOAT32_COL", "FLOAT64_COL", "STRING_COL", "DATE_COL", "SHAPE" };
	...
	public void setUp() throws IOException {
		...
		if (this.point_table == null) {
			throw new IllegalArgumentException("point_table not defined in "
					+ propsFile);
		}

		if (this.line_table == null) {
			throw new IllegalArgumentException("line_table not defined in "
					+ propsFile);
		}

		if (this.polygon_table == null) {
			throw new IllegalArgumentException("polygon_table not defined in "
					+ propsFile);
		}

		if (this.temp_table == null) {
			throw new IllegalArgumentException("temp_table not defined in "
					+ propsFile);
		}
	}
	...
	public void tearDown(boolean cleanTestTable, boolean cleanPool) {
		...
		if (cleanPool) {
			ArcSDEConnectionPoolFactory pfac = ArcSDEConnectionPoolFactory
					.getInstance();
			pfac.clear();
		}
	}
	...
	private static void insertData(SeLayer layer, SeConnection conn,
			SeColumnDefinition[] colDefs) throws Exception{
		...
		geoms[3] = reader
				.read("MULTILINESTRING((-180 -90, 180 90), (-180 90, 180 -90))");
		geoms[4] = reader
				.read("POLYGON((-10 -10, -10 10, 10 10, 10 -10, -10 -10))");
		geoms[5] = reader
	...
	private static MultiPoint createTestMultiPoint(GeometryFactory gf, int index) {
		Coordinate[] coords = { new Coordinate(index, index),
	...
	private static MultiPolygon createTestMultiPolygon(GeometryFactory gf,
			int index) {
		Polygon[] polys = { createTestPolygon(gf, index),
				createTestPolygon(gf, 1 + index) };
