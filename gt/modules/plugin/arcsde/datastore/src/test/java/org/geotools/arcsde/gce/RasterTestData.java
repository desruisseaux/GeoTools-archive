...
public class RasterTestData {
	 ...
	 */
	public String get1bitRasterTableName() throws Exception {
		return testData.getTemp_table() + "_ONEBITRASTER";
	}

	public String getRGBRasterTableName() throws Exception {
		return testData.getTemp_table() + "_RGBRASTER";
	}

	public String getRGBARasterTableName() throws Exception {
	...
	/**
	 * Loads the 1bit raster test data into the table given in {@link RasterTestData#get1bitRasterTableName()}
	 * 
	 * @throws Exception
	 */
	public void load1bitRaster() throws Exception {
		...
		importRasterImage(tableName, crs, rasterFilename, SeRaster.SE_PIXEL_TYPE_1BIT, imgExtent, producer);
	}
	...
	public void loadRGBRaster() throws Exception {
		...
		testData.deleteTable(tableName);
		// build the base business table. We'll add the raster data to it in a bit
		createRasterBusinessTempTable(getRGBRasterTableName(), conn);
		conn.close();
	...
	public void createRasterBusinessTempTable(String tableName, ArcSDEPooledConnection conn) throws Exception {
	...
	public void importRasterImage(final String tableName, SeCoordinateReference crs, final String rasterFilename, final int sePixelType, SeExtent extent, ArcSDERasterProducer prod) throws Exception {
		ArcSDEPooledConnection conn = testData.getConnectionPool().getConnection();
		try {

			// much of this code is from http://edndoc.esri.com/arcsde/9.2/concepts/rasters/dataloading/dataloading.htm
	...
	public SeRaster getRaster(final String rasterName) throws DataSourceException, UnavailableArcSDEConnectionException {

		try {
			SeRasterAttr rattr = getRasterAttributes(rasterName, new Rectangle(0, 0, 0, 0), 0, new int[] { 0 });
			return rattr.getRasterInfo();
		} catch (SeException se) {
	...
	public SeRasterAttr getRasterAttributes(final String rasterName, Rectangle tiles, int level, int[] bands) throws DataSourceException, UnavailableArcSDEConnectionException {

		ArcSDEPooledConnection conn = testData.getConnectionPool().getConnection(); 
		
		try {
			SeQuery query = new SeQuery(conn, new String[] { conn.getRasterColumn(rasterName).getName() }, new SeSqlConstruct(rasterName));
			conn.getLock().lock();
			query.prepareQuery();
			query.execute();
