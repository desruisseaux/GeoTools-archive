			...
			while (fr1.hasNext())
			{
				...
				try {
    				while (fr2 != null && fr2.hasNext())
    				{
    					...
    					{
    						results.error( f1, f1.getPrimaryGeometry().getGeometryType()+" "+getGeomTypeRefA()+" within "+getGeomTypeRefB()+"("+f2.getID()+"), Result was not "+expected );
    						success = false;
    					}
    				}
                }
				...
				try {
    				while (fr2 != null && fr2.hasNext())
    				{
    					...
    					if (!f1.getID().equals(f2.getID()))	// if they are the same feature, move onto the next one
    					{
    						if(g1.within(g2) != expected )
    						{
    							results.error( f1, f1.getPrimaryGeometry().getGeometryType()+" "+getGeomTypeRefA()+" within "+getGeomTypeRefA()+"("+f2.getID()+"), Result was not "+expected );
    							success = false;
    						}
    					}
    				}
