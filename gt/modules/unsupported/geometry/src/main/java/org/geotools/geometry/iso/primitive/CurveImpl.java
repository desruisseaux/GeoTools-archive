...
public class CurveImpl extends OrientableCurveImpl implements Curve {
	...
	public List asLineSegments() {
		...
		ArrayList<LineSegmentImpl> mergedSegments = new ArrayList<LineSegmentImpl>();
		for (CurveSegment curveSegment : this.curveSegments) {
			LineString lineString = curveSegment.asLineString(0, 0);
			List<LineSegmentImpl> lineSegments = lineString.asLineSegments();
			for (LineSegmentImpl lineSegment : lineSegments)
...
//		}
	...
	public DirectPosition getRepresentativePoint() {
		...
		// old:
		//return ((CurveBoundaryImpl) this.getBoundary()).getStartPoint().getPosition();
		/* Return Position in the middle of the curve */
		// return this.forParam(0.5 * this.length());
	}
	...
	public Complex getClosure() {
		// Return a CompositeCurve which is created by this curve
		List<OrientableCurve> curveList = new ArrayList<OrientableCurve>();
		curveList.add(this);
		return this.getGeometryFactory().getComplexFactory().createCompositeCurve(curveList);
	}
	...
	public DirectPosition getCentroid() {
		...
		// this.getGeometryFactory().getCoordinateFactory().createDirectPosition();
		// /* Loop all Segments (CurveSegment´s) of the Curve */
		// for (CurveSegment seg : this.curveSegments) {
		// double length = ((CurveSegmentImpl) seg).length();
		// /* Get Centroids of each CurveSegment */
		// DirectPositionImpl center = ((CurveSegmentImpl) seg).getCentroid()
		// .scale(length);
		// // System.out.println(seg.centroid());
		// /* Add all Centroids of CurveSegments */
		// centroid = centroid.add(center);
		// }
		// /* Divide the summed Centroids by number of CurveSegments */
		// centroid = centroid.scale(1.0 / this.curveSegments.size());
		// /*
		// * Divide by average length of all CurveSegments and return result as
		// * centroid of the Curve
		// */
		// return centroid.scale(1.0 / (this.length() /
		// this.curveSegments.size()));
		return null;
	}
	...
	public CurveImpl merge(CurveImpl other) {
		...
		if (this.getStartPoint().equals(other.getEndPoint())) {
			firstCurve = other;
			secondCurve = this;
		} else
		...
		List<DirectPosition> newDPList = new ArrayList<DirectPosition>();
		List<DirectPositionImpl> dPList = firstCurve.asDirectPositions();
		for (int i=0; i<dPList.size(); i++) {
			newDPList.add(dPList.get(i).clone());
		}
		dPList = secondCurve.asDirectPositions();
