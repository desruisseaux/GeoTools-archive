 ...
 * 
 */
public class MultiContextTreeTransferHandler extends TransferHandler {

    
    ArrayList<MultiMetaTransfer> metaTransfers = null;

    /**
     * constructor
     */
    public MultiContextTreeTransferHandler() {
        super();
    }
    ...
    protected Transferable createTransferable(JComponent c) {
        ...
        if (c instanceof JXTreeTable) {
            ...
            for (int i = 0; i < nbSelected; i++) {
                ...
                MultiMetaTransfer mt = new MultiMetaTransfer();
            ...
            if (metaTransfers.size() > 0) {
                ...
                for (MultiMetaTransfer mt : metaTransfers) {
            ...
            t = new MultiContextTreeTransferable(metaTransfers);
        }
        return t;
    }
    ...
    protected void addNodes(TreePath currentPath, ContextTreeModel model, Transferable data) {
        ...
        try {
            List<MultiMetaTransfer> metatransfers = (List<MultiMetaTransfer>) data.getTransferData(DataFlavor.stringFlavor);

            for (MultiMetaTransfer metatransfer : metatransfers) {
    ...
    protected void insertNodes(JXTreeTable treeTable, ContextTreeModel model, Transferable data) {
        ...
        Point location = ((MultiContextTreeDrop) treeTable.getDropTarget()).getMostRecentDragLocation();
