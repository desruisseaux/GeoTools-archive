 ...
 */
public class JMapPane2D extends JPanel {
     ...
     */
    public JMapPane2D(GTRenderer render, MapContext context) {
     ...
     *            what to draw
     */
    public JMapPane2D(LayoutManager layout, boolean isDoubleBuffered,GTRenderer render, MapContext context) {
    ...
    void processDrag(int x1, int y1, int x2, int y2) {
        ...
        if (state == STATE.PAN) {
            ...
            mapArea = fixAspectRatio(this.getBounds(), new Envelope(ll, ur));
        } else if (state == STATE.SELECT && selectionLayer !=null) {
            double left = Math.min(startX, endX);
            double right = Math.max(startX, endX);
            double bottom = Math.min(startY, endY);
            double top = Math.max(startY, endY);
...
class MouseListen implements MouseListener,MouseMotionListener{
    private final JMapPane2D map;    
    MouseListen(JMapPane2D map){
        this.map = map;
    }
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if ((map.getState() == JMapPane2D.STATE.ZOOM_IN) || (map.getState() == JMapPane2D.STATE.ZOOM_OUT)) {
    ...
    public void mouseDragged(MouseEvent e) {   
        ...
        if (map.state == JMapPane2D.STATE.PAN) {
            // move the image with the mouse
            ...
            map.lastX = x;
            map.lastY = y;
        } else if ((map.state == JMapPane2D.STATE.ZOOM_IN) || (map.state == JMapPane2D.STATE.ZOOM_OUT)) {
            ...
            map.lastY = y;
            map.drawRectangle(graphics);
        } else if (map.state == JMapPane2D.STATE.SELECT && map.selectionLayer != null) {
...
class MapLayerListListen implements MapLayerListListener{
    private final JMapPane2D map;    
    MapLayerListListen(JMapPane2D map){
        this.map = map;
    }
...
class HighlightChangeListen implements HighlightChangeListener{
    private final JMapPane2D map;    
    HighlightChangeListen(JMapPane2D map){
        this.map = map;
    }

    public void highlightChanged(org.geotools.gui.swing.map.map2d.temp.event.HighlightChangedEvent e) {   
...
class SelectionChangeListen implements SelectionChangeListener{
    private final JMapPane2D map;    
    SelectionChangeListen(JMapPane2D map){
        this.map = map;
    }
