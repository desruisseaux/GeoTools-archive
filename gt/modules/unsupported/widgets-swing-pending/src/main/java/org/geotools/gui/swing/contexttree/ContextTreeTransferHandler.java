...
public class ContextTreeTransferHandler extends TransferHandler{
    ...
    protected Transferable createTransferable(JComponent c) {
        ...
        if(c instanceof JXTreeTable) {
            ...
            t = new ContextTreeTransferable(treeTable.getPathForRow(treeTable.getSelectedRow()));
            dragPath = treeTable.getPathForRow(treeTable.getSelectedRow());
            
            dragPath.getLastPathComponent();
            
            if (dragPath != null) {
                draggedNode = (ContextTreeNode) dragPath.getLastPathComponent();
                
                ContextTreeNode parent = (ContextTreeNode) draggedNode.getParent();
                origine = parent.getIndex(draggedNode);
                origine_parent = (MapContext)parent.getUserObject();
            }
        }
        return t;
    }
    ...
    protected void insertNodes(JXTreeTable treeTable, ContextTreeModel model, Transferable data) {
        ...
        Point location = ((ContextTreeDrop) treeTable.getDropTarget()).getMostRecentDragLocation();
        ...
        try {
            ...
            if(!moveNode.equals(targetNode) && ! parent.isNodeAncestor(moveNode)) {
                ...
                if( (moveNode.getUserObject() instanceof MapLayer)  && (parent.getUserObject() instanceof MapContext) ){
                    ...
                    if( parent.getUserObject().equals(origine_parent) && place>origine )  place--;
    ...
    public BufferedImage getDragImage(JXTreeTable tree) {
        ...
        try {
            if (dragPath != null) {
                ...
                JComponent lbl = (JComponent)r.getTableCellRendererComponent(tree, draggedNode.toString(),false, false, row, 0);
