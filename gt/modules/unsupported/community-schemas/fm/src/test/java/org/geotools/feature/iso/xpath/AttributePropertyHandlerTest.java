...
public class AttributePropertyHandlerTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		tb = (TypeBuilder) 
			container.getComponentInstanceOfType(TypeBuilder.class);
		fb = (AttributeBuilder)
			container.getComponentInstanceOfType(AttributeBuilder.class);
		
		TypeFactory tf = tb.getTypeFactory();
		AttributeType integerType = tf.createAttributeType(
			new TypeName("integerType"), Integer.class, false, false,
			null, null, null
		);
		AttributeType stringType = tf.createAttributeType(
			new TypeName("stringType"), String.class, false, false,
			null, null, null
		);
		AttributeType doubleType = tf.createAttributeType(
			new TypeName("doubleType"), Double.class, false, false,
			null, null, null
		);
	...
	public void testSimpleFeature() {
		...
		JXPathIntrospector.registerDynamicClass(
				FeatureImpl.class, AttributePropertyHandler.class);
