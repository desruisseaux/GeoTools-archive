...
/**
 * This test cases will check that the core type system is opperating
 * according to specification.
 * <p>
 * This test uses the *TestContainer* and makes no assumptions about
 * implementation. If you wish to subclass this you may reuse the
 * test methods with alternate implementations.
 * </p>
 * <p>
...
public class TypeSystemTest extends TestCase {
	 ...
	 * <p>
	 * Looks like we should make add( Type ) just make us happy,
	 * can always reject types that do not agree with the schema uri.
	 * </p>
	 */
	public void testSchema(){
		...
		AttributeType TEXT = f.createAttributeType( 
			typeName(URI,"Text"), String.class, false, false, Collections.EMPTY_SET, null, null 
		);
		AttributeType NUMBER = f.createAttributeType( 
			typeName(URI,"Number"), Integer.class, false, false, Collections.EMPTY_SET, null, null 
		);

		Schema schema = new org.geotools.feature.type.SchemaImpl( URI );
		try {
		...
		AttributeType EVIL = f.createAttributeType(
			typeName("/dev/null","Evil"), Void.class, false, false, Collections.EMPTY_SET, null,null 
		);
		try {
	...
	public void testTypeFactory(){
		...
		ComplexType ADDRESS = f.createComplexType(
				typeName("Person"), address, true, false, null, null, null );
		assertNotNull( ADDRESS  );
		assertEquals( "has attributes", 5, ADDRESS.attributes().size() );		

		AttributeType DATE = f.createAttributeType( typeName(URI,"Date"), Date.class, false, false, Collections.EMPTY_SET, null, null );
		AttributeType LOCATION = f.createGeometryType( typeName(URI,"Location"), Point.class, null, false, false, null, null, null );
		...
		FeatureType PERSON = f.createFeatureType(
				typeName("Person"), person,location, null, false, null, null, null );
	...
	static final Name name( String local ){
		return new org.geotools.util.Name( null, local );
	}
	static final Name name( String uri, String local ){
		return new org.geotools.util.Name( uri, local );
	}
	static final TypeName typeName( String uri, String type ){
		return new org.geotools.util.TypeName( uri, type );
	}
	...
	static final AttributeDescriptor attribute( String name, AttributeType type ){
		Name attributeName = new org.geotools.util.Name( "irc://localhost/#bot", name );
