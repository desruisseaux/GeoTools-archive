 ...
 * </p>
 * FIXME: These methods need a Q&A check to confirm correct use of Super
...
public class Types {
	 ...
	 * 
	 * @return The name in which getLocalPart() == name
	 * and getNamespaceURI() == null. Or null if name == null.
	 */
    public static Name attributeName(String name) {
    	if (name == null) {
    		return null;
    	}    
		return new org.geotools.util.Name(name);
    }
	 ...
	 * 
	 * @return The name in which getLocalPart() == name
	 * and getNamespaceURI() == namespace.
	 */
    public static Name attributeName(String namespace, String name) {
    	return new org.geotools.util.Name(namespace,name);
    }
    ...
    public static Name attributeName(Name name) {
    	return new org.geotools.util.Name(
    ...
    public static TypeName typeName(String name) {
		...
		return new org.geotools.util.TypeName(name);
    }
    ...
    public static TypeName typeName(String namespace, String name) {
    	return new org.geotools.util.TypeName(namespace,name);
    }	
    ...
    public static TypeName typeName(Name name) {
    	return new org.geotools.util.TypeName(
			name.getNamespaceURI(),name.getLocalPart()
		);
    }
    ...
    /**
    * Convenience method for turning an array of qualified names into 
    * a list of non qualified names.
    *  
    */
    ...
    */
	...
	{
		return descriptor(type, new org.geotools.util.Name(name));
	}
	...
	{
		return descriptor(type, new org.geotools.util.Name(namespace,name));
	}
	...
	/**
	 * Returns the set of descriptors matching the given name within the 
	 * given type.
	 *
	 * @param type The type, non null.
	 * @param name The name, non null.
	...
	public static List/*<PropertyDescriptor>*/ descriptors(
		ComplexType type, String name
	) {
		
		return descriptors(type, new org.geotools.util.Name(name));
	}
	 ...
	 * 
	 * @throws IllegalAttributeException In the event that content violates 
	 * any restrictions specified by the attribute.
	 */
	...
	public static void validate(
			AttributeType type, Attribute attribute, Object attributeContent
		) throws IllegalAttributeException {
	...
	protected static void validate(
		...
		if (!isSuper) {
			...
			// former, but their associated java bindings, (BigDecimal, and Integer)
			// dont.
			Class clazz = attributeContent.getClass();
			Class binding = type.getBinding();
			if (binding != null && !binding.isAssignableFrom(clazz)) {
				throw new IllegalAttributeException(
					clazz.getName() + " is not an acceptable class for "
					+ type.getName() + " as it is not assignable from " + binding
				);
			}
		}
	...
	protected static void validate(
		ComplexType type, ComplexAttribute attribute, Collection content
	) throws IllegalAttributeException {
		...
		for (Iterator itr = content.iterator(); itr.hasNext();) {
			...
			if (att == null) {
				throw new NullPointerException(
						"Attribute at index " + index + " is null. Attributes "
	...
	private static void validateSequence(
		SequenceType type, ComplexAttribute att, Collection content
	) throws IllegalAttributeException {
		
		if (!(att instanceof Sequence)) {
			throw new IllegalAttributeException(
				"Attribute must be instance of: " + Sequence.class.getName() + 
				" for type instance of: " + SequenceType.class.getName()
			);
		}
		...
		if (!(content instanceof List)) {
			throw new IllegalAttributeException(
				"Content must be instance of " + List.class.getName() + 
				" for Sequence"
			);
		}
	...
	private static void processSequence(
		...
		do {
			...
			if (occurences < min || occurences > max) {
				throw new IllegalAttributeException(
					"Found " + occurences + " occurences of " + ad.getName() + 
					" when between " + min + " and " + max + " expected"
				);
			}
	...
	private static void validateChoice(
		ChoiceType type, ComplexAttribute att, Collection content
	) throws IllegalAttributeException {
		
		if (!(att instanceof Choice)) {
			throw new IllegalAttributeException(
				"Attribute must be instance of: " + Choice.class.getName() + 
				" for type instance of: " + ChoiceType.class.getName()
			);
		}
	...
	private static void processChoice(
		Set/*<AttributeDescriptor>*/ choice, Collection/*<Attribute>*/ content
	) throws IllegalAttributeException {
	...
	private static void validateAll(
		ComplexType type, ComplexAttribute att, Collection content
	) throws IllegalAttributeException {
		processAll(type.attributes(), content);
	}
	
	private static void processAll (
		Collection/*<AttributeDescriptor>*/ all, Collection/*<Attribute>*/ content
	) throws IllegalAttributeException {
		...
		for (Iterator itr = all.iterator(); itr.hasNext();) {
			...
			if (occurences < ad.getMinOccurs() || occurences > ad.getMaxOccurs()) {
				throw new IllegalAttributeException(
					"Found " + occurences + " of " + ad.getName() + " when type"
