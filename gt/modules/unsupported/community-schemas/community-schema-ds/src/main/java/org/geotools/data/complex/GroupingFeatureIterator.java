...
class GroupingFeatureIterator extends AbstractMappingFeatureIterator {
	...
	/**
	 * Map of xpath target attributes/parent multivalued property.
	 * Used to recognize child properties of a multivalued and
	 * complex attributes and set their values for each member of the group.
	 */
	private Map /*<String, XPath.Step>*/ multivaluedAttributePaths = new HashMap(); 
	
	private static final AttributeFactory attf = new AttributeFactoryImpl();
	 ...
	 * @throws IOException
	 */
	public GroupingFeatureIterator(FeatureTypeMapping mappings, Query query)
			throws IOException {
		super(mappings, query);
	...
	protected Query getUnrolledQuery(Query query){
		...
		Query unmappedQuery = ComplexDataStore.unrollQuery(query, mapping);
	...
	/**
	 * Takes a Query and returns another one ensuring that all
	 * the grouping attributes are requested, in order to be able
	 * of producing the correct number of output features, for example,
	 * from a joined set of tables.
	 *  
	 * @param query
	 * @return
	 */
	...
	private Feature computeNext() {
		...
		String fid = extractIdForAttribute(name, this.curSrcFeature);

		// create the target feature and iterate in the source for set its
		// values.
		final Feature targetFeature = attf.createFeature(targetNode, fid);
		...
		for (Iterator itr = attMappings.iterator(); itr.hasNext();) {
			...
			if(mapping.isMultiValued()){
				...
				setSingleValuedAttribute(targetFeature, featureForGroupingAtts, mapping, currentGroup);
			}
		}
	...
	/**
	 * Iterate over the source features while they belong to the same set
	 * of grouping attributes.
	 * 
	 * @return the set of Features from the source resultset that belongs to the same
	 * group.
	 */
	 ...
	 */
	private void setSingleValuedAttribute(	Feature target, 
		...
		if (expression == null) {
			expression = FeatureTypeMapping.NULL_EXPRESSION;
		}
		...
		if(parentMultivaluedAttributePath != null){
			...
			for(Iterator it = currentGroup.iterator(); it.hasNext();){	
				...
				try{
					value = expression.getValue(sourceFeature);
				}catch(Exception e){
				...
				String id = extractIdForAttribute(xpath, sourceFeature);
				
				List childSteps = setIndexAtStep(xpath, index, parentIndex);
				
				String newXpath = XPath.Step.toXPathString(childSteps);
				
				Attribute instance = XPath.set(target, newXpath, value, id, targetNodeType);
			...
			try{
				value = expression.getValue(source);

				// if target has id construct the id value from source
				String id = extractIdForAttribute(xpath, this.curSrcFeature);
				Attribute instance = XPath.set(target, xpath, value, id, targetNodeType);
				Map clientPropsMappings = attMapping.getClientProperties();
	...
	/**
	 * Sets values of compex attributes, meaning those that are marked by
	 * the {@link AttributeMapping#isMultiValued() multivalued flag} on the 
	 * attribute mapping.
	 * 
	 * @param sourceFeature
	 ...
	 */
	private void setMultivaluedAttribute(Feature target, AttributeMapping attMapping, List/*<Feature>*/ group) {
		...
		if (sourceExpression == null) {
			sourceExpression = FeatureTypeMapping.NULL_EXPRESSION;
		}

		final FeatureType targetType = (FeatureType) target.getDescriptor().getType();
		...
		for (Iterator itr = group.iterator(); itr.hasNext();) {
			...
			try{
				value = sourceExpression.getValue(sourceFeature);
			}catch(Exception e){
				//HACK: what we actually need to resolve is dealing
				//with queries that restricts the attributes returned
				//by the source featurestore
			...
			index++;
			// if target has id construct the id value from source
			String id = extractIdForAttribute(targetXpath, sourceFeature);

			// if complex or leaf of complex attribute then insert index in
			// xpath.
			String targetXpathAttr = null;

			if (XPath.isComplexType(targetXpath, target.getDescriptor())) {
			...
			Attribute instance = XPath.set(target, targetXpathAttr, value, id, targetNodeType);
			Map clientPropsMappings = attMapping.getClientProperties();
			setClientProperties(instance, sourceFeature, clientPropsMappings);
		}
	}
	...
	private void setClientProperties(Attribute target, Feature source, Map clientProperties){
		...
		for(Iterator it = clientProperties.entrySet().iterator(); it.hasNext();){
			...
			org.opengis.feature.AttributeName propName = (org.opengis.feature.AttributeName)entry.getKey();
			Expression propExpr = (Expression)entry.getValue();
			
			Object propValue = propExpr.getValue(source);
			
			nodeAttributes.put(propName, propValue);
		}

		node.putClientProperty("attributes", nodeAttributes);
	}
	...
	private final void splitMappings(final List allAttrMappings) {
		...
		//make a list of all attribute names used by the source expression
		final List sourceAttNames = new LinkedList();
		FilterVisitor attExtractor = new AbstractFilterVisitor(){
			 public void visit(AttributeExpression expression) {
				 String attName = expression.getAttributePath();
				 sourceAttNames.add(attName);
			 }
		};
		...
		for (Iterator itr = allAttrMappings.iterator(); itr.hasNext();) {
			...
			sourceAttNames.clear();
			source.accept(attExtractor);
			...
			if(groupByAttributeNames.containsAll(sourceAttNames)){
				...
				LOGGER.fine("attributes of multivalued property: " + sourceAttNames);
		...
		for (Iterator itr = allAttrMappings.iterator(); itr.hasNext();) {
			...
			String targetXPath = attMapping.getTargetXPath();
			List targetSteps = XPath.steps(mapping.getTargetFeature(), targetXPath);

			for(Iterator mvalues = multivaluedAttributes.iterator(); mvalues.hasNext();){
				String multivalued = (String)mvalues.next();
				List parentSteps = XPath.steps(mapping.getTargetFeature(), multivalued);
	...
	private final boolean isLeafOfNestedComplexType(
		...
		if (!XPath.isLeaf(xpathAttrDefinition, targetFeatureNode))
			return false;

		// if leaf then check if its parent attribute is complex
		List stepList = XPath.steps(targetFeatureNode, xpathAttrDefinition);
		/*
	...
	private final String insertIndexInXpathOfComplex(
			final Attribute root,
			final String xpathAttrDefinition, int index) {

		List stepList = XPath.steps(root.getDescriptor(), xpathAttrDefinition);

		int insertPosition = stepList.size();

		String indexXpath = insertIndexInXpath(root,
				xpathAttrDefinition, index, insertPosition);
	 ...
	 */
	private final String insertIndexInXpathOfLeafAttr(
			final Attribute root, final String attrXpath,
			final int index) {

		List/*<XPath.Step>*/ stepList = XPath.steps(root.getDescriptor(), attrXpath);
	...
	/**
	 * 
	 * @param xpath XPath expression for which to set the <code>stepIndex</code>'th attribute
	 * to index <code>newInde</code>
	 * @param newIndex index to set for step number <code>stepIndex</code>. <code>newIndex</code>
	 * minimun value is <code>1</code>, as in XPath spec.
	 * @param stepIndex index of the step in the list of steps for <code>xpath</code> for which
	 * to set the step index to <code>newIndex</code>. The minimun value is <code>0</code>, as
	 * in Java collections and arrays.
	 * @return
	 */
	private final List/*<XPath.Step>*/ setIndexAtStep(String xpath, int newIndex, int stepIndex){
		List steps = XPath.steps(mapping.getTargetFeature(), xpath);
	...
	private final List/*<List<Attribute>>*/ extractGroupingAttributes(
		...
		for (Iterator itr = this.groupByAttributeNames.iterator(); itr.hasNext();) {
			...
			List/*<Attribute>*/ listAttrForName = 
				srcFeature.getAttributes(new AttributeName(attrName));
