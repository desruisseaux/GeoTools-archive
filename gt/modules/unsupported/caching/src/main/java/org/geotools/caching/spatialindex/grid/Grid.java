...
//		 TODO: handle case where shape is bigger than one tile ...
		...
		for (int i = 0 ; i < this.dimension ; i++) {
			...
			minindexes[i] = (int) ((shape.getMBR().getLow(i)-node.mbr.getLow(i))/node.tiles_size[i]) ;
			cursor[i] = minindexes[i] ;
			maxindexes[i] = (int) ((shape.getMBR().getHigh(i)-node.mbr.getLow(i))/node.tiles_size[i]) + 1 ;
		}
		do {
