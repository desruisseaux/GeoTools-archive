...
public class JDBCFeatureCollection extends AbstractContentFeatureCollection {
    ...
    protected FeatureIterator createFeatureWriter() throws Exception {
        ...
        ContentFeatureIterator iterator = new JDBCFeatureWriter(st, getFeatureSource().getSchema(),
                this);
        ...
        try {
            if ((postFilter != null) && (postFilter != Filter.INCLUDE)) {
                ...
                if (state.getBounds() == null) {
                    synchronized (state) {
                        if (state.getBounds() == null) {
                            state.setBounds(dataStore.getBounds(getSchema(), null,
                                    getState().getConnection()));
                        }
                    }
                }
        ...
        try {
            if ((postFilter != null) && (postFilter != Filter.INCLUDE)) {
                ...
                if (state.getCount() == -1) {
                    synchronized (state) {
                        if (state.getCount() == -1) {
                            state.setCount(dataStore.getCount(getSchema(), null,
                                    getState().getConnection()));
                        }
                    }
                }
    ...
    public void update(AttributeDescriptor[] attributes, Object[] values) {
        ...
        try {
            if (filter == null) {
                ...
                if (preFilter != null) {
                    //delete what we can up front
                    dataStore.update(getSchema(), attributes, values, preFilter,
                        getState().getConnection());
                }
