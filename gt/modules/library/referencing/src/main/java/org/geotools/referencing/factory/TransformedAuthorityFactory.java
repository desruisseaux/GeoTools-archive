...
/**
 * An authority factory which returns modified
 * {@linkplain CoordinateReferenceSystem CRS}, {@linkplain CoordinateSystem CS}
 * or {@linkplain Datum datum} objects from other factory implementations. This
 * class provides a set of {@code replace(...)} methods to be overriden by
 * subclasses in order to replace some
 * {@linkplain CoordinateReferenceSystem CRS}, {@linkplain CoordinateSystem CS}
 * or {@linkplain Datum datum} objects by other ones. The replacement rules are
 * determined by the subclass being used. For example the
 ...
 * <p>
 * All constructors are protected because this class must be subclassed in order
 * to determine which of the {@link DatumAuthorityFactory},
 * {@link CSAuthorityFactory} and {@link CRSAuthorityFactory} interfaces to
 * implement.
 * 
 * @since 2.3
 * @source $URL:
 *         http://svn.geotools.org/geotools/trunk/gt/module/referencing/src/org/geotools/referencing/factory/TransformedAuthorityFactory.java $
 * @version $Id: TransformedAuthorityFactory.java 19962 2006-06-14 07:41:54Z
 *          desruisseaux $
...
public class TransformedAuthorityFactory extends AuthorityFactoryAdapter {
    /**
     * A set of default factories to be used if none were found if
     * {@link #datumFactory}, {@link #csFactory}, {@link #crsFactory} or
     * {@link #operationFactory}. Will be created only when first needed.
     */
     ...
     * @param csFactory
     *            The {@linkplain CoordinateSystem coordinate system} authority
     *            factory, or {@code null}.
     * @param datumFactory
     *            The {@linkplain Datum datum} authority factory, or
     *            {@code null}.
     * @param opFactory
     *            The {@linkplain CoordinateOperation coordinate operation}
     *            authority factory, or {@code null}.
     */
     ...
     * 
     * @param authority
     *            The authority to wraps (example: {@code "EPSG"}). If
     *            {@code null}, then all authority factories must be explicitly
     *            specified in the set of hints.
     * @param hints
    ...
    TransformedAuthorityFactory(final String authority, final Hints hints)
        ...
        super(FactoryFinder.getCRSAuthorityFactory(authority, hints),
                ...
                FactoryFinder.getCoordinateOperationAuthorityFactory(authority,
                        hints));
    }

    /**
     * Suggests a factory group to use for the specified authority factory. The
     * {@code factory} parameter is usually one of {@link #datumFactory},
     * {@link #csFactory}, {@link #crsFactory} or {@link #operationFactory}
     * field. It may be {@code null}.
     */
    ...
    /**
     * Returns the implementation hint for the specified factory, or
     * {@code null} if none.
     */
    private static Object getHint(final AuthorityFactory factory,
            final Hints.Key key) {
    ...
    private final CoordinateOperationFactory getCoordinateOperationFactory() {
        if (opFactory == null) {
            final Object candidate = getHint(operationFactory,
    ...
    protected CoordinateReferenceSystem replace(
        ...
        if (crs instanceof GeneralDerivedCRS) {
            ...
            final CoordinateReferenceSystem oldBaseCRS = derivedCRS
                    .getBaseCRS();
            ...
            fromBase = new DefiningConversion(getProperties(fromBase), fromBase
                    .getMethod(), fromBase.getParameterValues());
    ...
    private CoordinateSystem createCS(final Class/* <CoordinateSystem> */type,
        ...
        if (CartesianCS.class.isAssignableFrom(type)) {
            ...
            case 2:
                return csFactory
                        .createCartesianCS(properties, axis[0], axis[1]);
            case 3:
                return csFactory.createCartesianCS(properties, axis[0],
                        axis[1], axis[2]);
            }
        } else if (EllipsoidalCS.class.isAssignableFrom(type)) {
            switch (dimension) {
            case 2:
                return csFactory.createEllipsoidalCS(properties, axis[0],
                        axis[1]);
            case 3:
                return csFactory.createEllipsoidalCS(properties, axis[0],
                        axis[1], axis[2]);
            ...
            switch (dimension) {
            case 3:
                return csFactory.createSphericalCS(properties, axis[0],
                        axis[1], axis[2]);
            }
        } else if (CylindricalCS.class.isAssignableFrom(type)) {
            switch (dimension) {
            case 3:
                return csFactory.createCylindricalCS(properties, axis[0],
                        axis[1], axis[2]);
            }
            ...
            switch (dimension) {
            case 2:
                return csFactory.createUserDefinedCS(properties, axis[0],
                        axis[1]);
            case 3:
                return csFactory.createUserDefinedCS(properties, axis[0],
                        axis[1], axis[2]);
        ...
        throw new FactoryException(Errors.format(
                ErrorKeys.UNSUPPORTED_COORDINATE_SYSTEM_$1, Utilities
                        .getShortName(type)));
    }
