...
class ContrastEnhancementNode extends StyleVisitorCoverageProcessingNodeAdapter
	 ...
	 */
	private final static Set SUPPORTED_HE_ALGORITHMS;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.geotools.renderer.gridcoverage.StyleVisitorAdapter#visit(org.geotools.styling.ContrastEnhancement)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.geotools.renderer.gridcoverage.StyleVisitorCoverageProcessingNodeAdapter#execute()
	 */
	protected GridCoverage2D execute() {
		...
		if (sources != null || !sources.isEmpty()) {
			final GridCoverage2D source = getSource(0).getOutput();
			...
			if ((!Double.isNaN(gammaValue) && !Double.isInfinite(gammaValue) && !(gammaValue == 1 - 0))
				...
				////
				//
				// @todo HACK we need to convert to byte the image when going to
