...
/**
 * A helper class for rendering {@link GridCoverage} objects. Support for grid coverage SLD stylers
 * is still limited.
 * 
...
public final class GridCoverageRenderer {
    ...
    public GridCoverageRenderer( final CoordinateReferenceSystem destinationCRS,
            final Envelope envelope, Rectangle screenSize ) throws TransformException,
     ...
     * @param envelope delineating the area to be rendered.
     * @param screenSize at which we want to rendere the source {@link GridCoverage2D}.
     * @param java2dHints to control this rendering process.
     * @throws TransformException
     * @throws NoninvertibleTransformException
     */
    public GridCoverageRenderer( final CoordinateReferenceSystem destinationCRS,
    ...
    public void paint( final Graphics2D graphics, final GridCoverage2D gc,
            final RasterSymbolizer symbolizer ) throws FactoryException, TransformException,
            NoninvertibleTransformException {
        ...
        if (LOGGER.isLoggable(Level.FINE))
            LOGGER.fine(new StringBuffer("Drawing coverage ").append(gc.toString()).toString());
        ...
        final MathTransform GCCRSToDeviceCRSTransform = StreamingRenderer.getMathTransform(
                sourceCoverageCRS, destinationCRS);
        final MathTransform deviceCRSToGCCRSTransform = GCCRSToDeviceCRSTransform.inverse();
        if (LOGGER.isLoggable(Level.FINE))
            LOGGER.fine(new StringBuffer("Transforming coverage envelope with transform ").append(
        ...
        //
        // /////////////////////////////////////////////////////////////////
        final GridCoverage2D croppedGridCoverage = getCroppedCoverage(gridCoverage,
                destinationEnvelopeInSourceGCCRS, sourceCoverageCRS);

        if (croppedGridCoverage == null) {
            if (LOGGER.isLoggable(Level.FINE))
                LOGGER.fine(new StringBuffer(
                        "Skipping current coverage because cropped to an empty area").toString());
            return;
        }
        ...
        final double actualScaleX = lonFirst
                ? croppedCoverageGridToWorldTransform.getScaleX()
                : croppedCoverageGridToWorldTransform.getShearY();
        final double actualScaleY = lonFirst
                ? croppedCoverageGridToWorldTransform.getScaleY()
                : croppedCoverageGridToWorldTransform.getShearX();
        final double scaleX = actualScaleX
                / (lonFirst ? finalGridToWorldInGCCRS.getScaleX() : finalGridToWorldInGCCRS
                        .getShearY());
        final double scaleY = actualScaleY
                / (lonFirst ? finalGridToWorldInGCCRS.getScaleY() : finalGridToWorldInGCCRS
                        .getShearX());
        if (LOGGER.isLoggable(Level.FINE))
            LOGGER.fine(new StringBuffer("Scale factors are ").append(scaleX).append(" ").append(
                    scaleY).toString());
        ...
        if (LOGGER.isLoggable(Level.FINE))
            LOGGER.fine(new StringBuffer("Using interpolation ").append(interpolation).toString());
        // //
        //
        ...
        if (scaleX * scaleY <= 1.0) {
            ...
            // //
            if (LOGGER.isLoggable(Level.FINE))
                LOGGER.fine(new StringBuffer("Filtered subsample with factors ").append(scaleXInt)
                        .append(scaleYInt).toString());
            final GridCoverage2D preScaledGridCoverage = filteredSubsample(croppedGridCoverage,
                    scaleXInt, scaleYInt, new InterpolationNearest(), BorderExtender
            ...
            // //
            if (LOGGER.isLoggable(Level.FINE))
                LOGGER.fine(new StringBuffer("Scale down with factors ").append(scaleX * scaleXInt)
                        .append(scaleY * scaleYInt).toString());
            ...
            if (scaleX * scaleXInt == 1.0 && scaleY * scaleYInt == 1.0)
                ...
                scaledGridCoverage = scale(scaleX * scaleXInt, scaleY * scaleYInt, 0f, 0f,
            ...
            // ///////////////////////////////////////////////////////////////////
            if (!GCCRSToDeviceCRSTransform.isIdentity()) {
                preSymbolizer = resample(scaledGridCoverage, destinationCRS, interpolation == null
            ...
            if (!GCCRSToDeviceCRSTransform.isIdentity()) {
                reprojectedCoverage = resample(croppedGridCoverage, destinationCRS,
                        interpolation == null ? new InterpolationBilinear() : interpolation);
                if (LOGGER.isLoggable(Level.FINE))
                    LOGGER.fine(new StringBuffer("Reprojecting to crs ").append(
                            destinationCRS.toWKT()).toString());
            ...
            // ///////////////////////////////////////////////////////////////////
            if (LOGGER.isLoggable(Level.FINE))
                LOGGER.fine(new StringBuffer("Scale up with factors ").append(scaleX)
                        .append(scaleY).toString());
            preSymbolizer = (GridCoverage2D) scale(scaleX, scaleY, 0f, 0f, interpolation == null
                    ? new InterpolationBilinear()
                    : interpolation, BorderExtender.createInstance(BorderExtender.BORDER_COPY),
                    reprojectedCoverage);
        ...
        final AffineTransform finalGCgridToWorld = new AffineTransform(
                (AffineTransform) ((GridGeometry2D) recoloredGridCoverage.getGridGeometry())
                        .getGridToCRS2D());
        ...
        //
        // //
        final AffineTransform cloneFinalWorldToGrid = (AffineTransform) finalWorldToGrid.clone();
     ...
     * @return
     */
    private GridCoverage2D scale( final double scaleX, final double scaleY, float xTrans,
     ...
     * @return
     */
    private GridCoverage2D resample( final GridCoverage2D gc, CoordinateReferenceSystem crs,
     ...
     * @return
     */
    private GridCoverage2D filteredSubsample( final GridCoverage2D gc, int scaleXInt,
        ...
        if (scaleXInt == 1 && scaleYInt == 1)
            ...
            if (hints.get(JAI.KEY_INTERPOLATION) != null
                    && hints.get(JAI.KEY_INTERPOLATION).equals(new InterpolationNearest()))
     ...
     * @return
     */
    private GridCoverage2D getCroppedCoverage( GridCoverage2D gc, GeneralEnvelope envelope,
        ...
        if (!intersectionEnvelope.equals((GeneralEnvelope) oldEnvelope)) {
            ...
            tempGC = (GridCoverage2D) coverageCropFactory.doOperation(param, hints);
        } else
            tempGC = gc;
