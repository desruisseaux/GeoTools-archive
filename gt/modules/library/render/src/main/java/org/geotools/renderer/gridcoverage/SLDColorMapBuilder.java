...
public class SLDColorMapBuilder {
	...
	private final List colormapElements = new ArrayList();
	...
	private final List preservedValues = new ArrayList();
...
//			colorsToDistribute-=2;
	...
	public double[] getValuesToPreserve() {
		if (this.preservedValues.size() == 0)
			return null;
		final Iterator it = this.preservedValues.iterator();
		final double retVal[] = new double[this.preservedValues.size()];
		int i = 0;
		while (it.hasNext())
			retVal[i++] = ((Double) it.next()).doubleValue();
		return retVal;
	}
	...
	public SLDColorMapBuilder addValueToPreserve(final double value) {
		...
		preservedValues.add(new Double(value));
		return this;
	}
	...
	public SLDColorMapBuilder setColorForValuesToPreserve(final Color color) {
		...
		checkIfColorMapCreated();
		//@todo check number of color map entries
