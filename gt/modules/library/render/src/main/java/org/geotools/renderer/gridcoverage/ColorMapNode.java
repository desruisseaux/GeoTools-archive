...
class ColorMapNode extends StyleVisitorCoverageProcessingNodeAdapter implements
	...
	public synchronized void visit(ColorMap colorMap) {
		...
		if (cmEntries != null && cmEntries.length > 0) {
			...
			final GridCoverage2D sourceCoverage = source.getOutput();
			...
			if (candidateNoDataValues == null)
				candidateNoDataValues = new double[] { Double.NaN };
	...
	protected GridCoverage2D execute() {
		...
		final GridCoverage2D sourceCoverage = sourceNode.getOutput();
		...
		if (classification != null) {
			...
			pbj.add(classification);
			final RenderedOp d = JAI.create(RasterClassifier.OPERATION_NAME,
					pbj);
			return getCoverageFactory().create("BandMerge", d,
					getSource(0).getOutput().getEnvelope());
		}
		return getCoverageFactory().create("color_mapped_"+sourceCoverage.getName().toString(),
				sourceImage,sourceCoverage.getEnvelope(),sourceCoverage.getSampleDimensions(),new GridCoverage[]{sourceCoverage},new HashMap(sourceCoverage.getProperties()));
