 ...
 * @since 2.1
 * @source $URL:
 *         http://svn.geotools.org/geotools/trunk/gt/module/referencing/src/org/geotools/factory/Hints.java $
 * @version $Id$
...
public final class Hints extends RenderingHints {
	...
	/**
	 * The {@link com.vividsolutions.jts.geom.CoordinateSequenceFactory}
	 * instance to use.
	 * 
	...
	/**
	 * The {@link org.opengis.referencing.crs.CRSAuthorityFactory} instance to
	 * use.
	 * 
	...
	/**
	 * The {@link org.opengis.referencing.cs.CSAuthorityFactory} instance to
	 * use.
	 * 
	...
	/**
	 * The {@link org.opengis.referencing.datum.DatumAuthorityFactory} instance
	 * to use.
	 * 
	...
	/**
	 * The {@link org.opengis.referencing.operation.MathTransformFactory}
	 * instance to use.
	 * 
	...
	/**
	 * The {@link org.opengis.coverage.processing.GridCoverageProcessor}
	 * instance to use.
	 * 
	 ...
	 * Valid values are {@code "Molodenski"}, {@code "Abridged_Molodenski"} or
	 * {@code "Geocentric"}. Other values may be supplied if a
	 * {@linkplain org.opengis.referencing.operation.MathTransform math
	...
	/**
	 * Tells if
	 * {@linkplain org.opengis.referencing.operation.CoordinateOperation coordinate
	 * operations} should be allowed even when a datum shift is required while
	 ...
	 * {@link Boolean#FALSE FALSE}, which means that
	 * {@linkplain org.geotools.referencing.operation.DefaultCoordinateOperationFactory coordinate
	 * operation factory} throws an exception if such a case occurs. If this
	 * hint is set to {@code TRUE}, then the user is strongly encouraged to
	 * check the
	 * {@linkplain org.opengis.referencing.operation.CoordinateOperation#getPositionalAccuracy
	 * positional accuracy} for every transformation created. If the set of
	 * positional accuracy contains
	 * {@link org.geotools.metadata.iso.quality.PositionalAccuracyImpl#DATUM_SHIFT_OMITTED
	 * DATUM_SHIFT_OMITTED}, this means that an "ellipsoid shift" were applied
	 * without real datum shift method available, and the transformed
	 * coordinates may have one kilometer error. The application should warn the
	 * user (e.g. popup a message dialog box) in such case.
	 * 
	 * @see org.geotools.referencing.FactoryFinder#getCoordinateOperationFactory
	 */
	 ...
	 * created by an
	 * {@linkplain org.opengis.referencing.cs.CSAuthorityFactory authority factory}
	 * should be forced to (<var>longitude</var>,<var>latitude</var>) axis
	 * order. This hint is especially useful for creating
	 ...
	 * {@linkplan org.opengis.referencing.crs.GeographicCRS geographic CRS}
	 * defined in the EPSG database use (<var>latitude</var>,<var>longitude</var>)
	 * axis order. Unfortunatly, many data sources available in the world uses
	 * the opposite axis order and still claim to use a CRS described by an EPSG
	 * code. This hint allows to handle such data.
	 * <p>
	 ...
	 * method. Whatever this hint is supported or not is authority dependent. In
	 * the default Geotools configuration, this hint is supported for the
	 * {@code "EPSG"} authority.
	 * <p>
	 * If this hint is not provided, then the default value depends on many
	 * factors including {@linkplain System#getProperties system properties} and
	 * plugins available in the classpath. In Geotools implementation, the
	 * default value is usually {@link Boolean#FALSE FALSE} with one exception:
	 * If the <code>{@value
	 * org.geotools.referencing.factory.epsg.LongitudeFirstFactory#SYSTEM_DEFAULT_KEY}</code>
	 * system property is set to {@code true}, then the default value is
	 * {@code true} at least for the
	 * {@linkplain org.geotools.referencing.factory.epsg.DefaultFactory default EPSG
	 * factory}.
	 * <p>
	 ...
	 * If {@code true}, then
	 * {@linkplain org.opengis.referencing.cs.AxisDirection#SOUTH South} axis
	 * directions are forced to
	 * {@linkplain org.opengis.referencing.cs.AxisDirection#NORTH North},
	 * {@linkplain org.opengis.referencing.cs.AxisDirection#WEST West} axis
	 * directions are forced to
	 * {@linkplain org.opengis.referencing.cs.AxisDirection#EAST East},
	 * <cite>etc.</cite> If {@code false}, then the axis directions are left
	 * unchanged.
	 * <p>
	 * This hint shall be passed to the
	 * <code>{@linkplain org.geotools.referencing.FactoryFinder#getCRSAuthorityFactory
	 ...
	 * If {@code true}, then all angular units are forced to degrees and linear
	 * units to meters. If {@code false}, then the axis units are left
	 * unchanged.
	 * <p>
	 * This hint shall be passed to the
	 * <code>{@linkplain org.geotools.referencing.FactoryFinder#getCRSAuthorityFactory
	...
	public static final class Key extends RenderingHints.Key {
		...
		/**
		 * Base class of all values for this key. Will be created from
		 * {@link #className} only when first required, in order to avoid too
		 * early class loading. This is significant for the
		 * {@link #JAI_INSTANCE} key for example, in order to avoid JAI
		 * dependencies in applications that do not need it.
		 */
