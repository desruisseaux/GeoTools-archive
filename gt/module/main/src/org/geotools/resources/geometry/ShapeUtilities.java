...
public final class ShapeUtilities {
     ...
     * qui se trouve le plus pr�s du point <code>(x,y)</code> sp�cifi�. Appellons
     * <code>result</code> le point retourn� par cette m�thode. Il est garanti
     * que <code>result</code> r�pond aux conditions suivantes (aux erreurs
     * d'arrondissements pr�s):
     *
     * <ul>
     *   <li><code>result</code> est un point du segment de droite
     *       <code>(x1,y1)-(x2,y2)</code>. Il ne trouve pas au del� des points
     *       extr�mes <code>(x1,y1)</code> et <code>(x2,y2)</code> de ce segment.</li>
    ...
    /**
     * Retourne le point sur le segment de droite <code>(x1,y1)-(x2,y2)</code>
     * qui se trouve � la distance <code>distance</code> sp�cifi�e du point
     * <code>point</code>. Appellons <code>result</code> le point retourn� par
     * cette m�thode. Si <code>result</code> est non-nul, alors il est garantit
     * qu'il r�pond aux conditions suivantes (aux erreurs d'arrondissements pr�s):
     *
     * <ul>
     *   <li><code>result</code> est un point du segment de droite <code>(x1,y1)-(x2,y2)</code>.
     *       Il ne trouve pas au del� des points extr�mes <code>(x1,y1)</code> et
        ...
        throws IllegalArgumentException
    {
        ...
        switch (orientation) {
            case PARALLEL: {
                ...
                if (!(check <= 1/EPS)) return null; // Deux points ont les m�mes coordonn�es.
                if (!(check >=   EPS)) return null; // Les trois points sont colin�aires.
                /*
