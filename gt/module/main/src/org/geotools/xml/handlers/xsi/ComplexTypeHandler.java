...
public class ComplexTypeHandler extends XSIElementHandler {
        ...
        if (child instanceof SimpleContentHandler
            ...
            if (child instanceof SimpleContentHandler) {
                ...
                if (sch.getChild() instanceof ExtensionHandler) {
                    ...
                    if ((rest.getBase() == null)
                            || rest.getBase().equalsIgnoreCase("")) {
                        st = ((SimpleTypeHandler) rest.getChild()).compress(parent);
                    } else {
                        st = parent.lookUpSimpleType(rest.getBase());
                    }

                    st = new RestrictedSimpleType(st, rest);
                    dct.parent = st;
                    dct.simple = true;
                }
     ...
     * 
     * <p> 
     * SimpleType implementation for restrictions
     * </p>
     * @see SimpleType
     * @author dzwiers
     *
     */
    private static class RestrictedSimpleType implements SimpleType {
        
        private SimpleType parent;
        private FacetHandler[] constraints;
        
        String namespace;

        /**
         * Applies the specified restriction to the simpletype provided
         * 
         * @param prt
         * @param rest
         */
        ...
        public Object getValue(Element element, ElementValue[] value,
            ...
            if ((value == null) || (value.length != 1)) {
                throw new SAXException(
                    "simple types must have one, and only one, text element");
            }

            return parent.getValue(element, value, attrs,hints);
        }
        ...
        /*
         * helper for getValue in applying the new restrictions
         */
        private void checkValue(ElementValue orgValue,
            ElementValue returnedValue) throws SAXException {
            if (constraints.length == 0) {
                return;
            }

            String val = (String) orgValue.getValue();
            ...
            if (constraints[0].getType() == FacetHandler.ENUMERATION) {
                ...
                throw new SAXException(
                    "was not one of the allowable enumerations");
            } else {
                Number nval = null;
                Date dval = null;
                String sval = val;
                ...
                if (returnedValue instanceof Number) {
                    nval = (Number) returnedValue;
                }

                if (returnedValue instanceof Date) {
                    dval = (Date) returnedValue;
                }
