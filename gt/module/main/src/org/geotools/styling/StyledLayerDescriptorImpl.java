 ...
 *     hope that specifications are always adaptive, and will be forced to throw
 *     unsupported exceptions when functionality is removed from a specification.
 * </ul>
 * </p>
 * @author jgarnett
 */
public class StyledLayerDescriptorImpl extends AbstractGTComponent implements StyledLayerDescriptor {
    /**
     ...
     */
    private synchronized void fire(GTEvent event) {
    	assert event != null;
    ...
    protected synchronized void fire(GTDelta delta) {
    	...
    	GTEventImpl event = new GTEventImpl();
    	event.setDelta( delta );
    	event.setRoot( this );
    	event.setType( GTEvent.Type.POST_CHANGE );
	...
	public void removed( GTDelta childDelta ){
		...
		GTDelta delta = new GTDeltaImpl( GTDelta.Kind.NO_CHANGE, this, childDelta );
		
	    GTEventImpl event = new GTEventImpl();
    	event.setDelta( delta );
    	event.setRoot( this );
    	event.setType( GTEvent.Type.PRE_DELETE );
