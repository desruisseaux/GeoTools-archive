...
/**
 * Base class of all coverage type. The essential property of coverage is to be
 * able to generate a value for any point within its domain. How coverage is
 * represented internally is not a concern. For example consider the following
 ...
 * </ul>
 * 
 * {@linkplain org.geotools.coverage.grid.GridCoverage2D Grid coverages} are
 * typically 2D while other coverages may be 3D or 4D. The dimension of grid
 * coverage may be queried in many ways:
 * 
 * <ul>
 ...
 * </ul>
 * 
 * All those methods should returns the same number. Note that the dimension of
 * grid coverage <strong>is not the same</strong> than the number of sample
 * dimensions. The later may be better understood as the number of bands for 2D
 * grid coverage.
 * 
 * @since 2.1
 * @source $URL:
 *         http://svn.geotools.org/geotools/trunk/gt/module/coverage/src/org/geotools/coverage/AbstractCoverage.java $
 * @version $Id$
 * @author Martin Desruisseaux
 */
public abstract class AbstractCoverage extends PropertySourceImpl implements
		Coverage {
	...
	protected AbstractCoverage(final CharSequence name,
			final CoordinateReferenceSystem crs, final PropertySource source,
	 ...
	 * 
	 * @param name
	 *            The name for this coverage, or {@code null} for the same than
	 *            {@code coverage}.
	 * @param coverage
	 *            The source coverage.
	 */
	protected AbstractCoverage(final CharSequence name, final Coverage coverage) {
		super(
				null,
				(coverage instanceof PropertySource) ? (PropertySource) coverage
						: null);
		...
		if (coverage instanceof AbstractCoverage) {
			...
			this.name = (n != null) ? n : new SimpleInternationalString(
					coverage.toString());
	 ...
	 * methods. It is also the coordinate reference system of the coordinates
	 * used with the math transform
	 * {@link GridGeometry#getGridToCoordinateSystem}. A grid coverage can be
	 * accessed (re-projected) with new coordinate reference system with the
	 * {@link GridCoverageProcessor} component. In this case, a new instance of
	 * a grid coverage is created.
	 * <p>
	 * Note: If a coverage does not have an associated coordinate reference
	 * system, the returned value will be {@code null}. The
	 * {@link GridGeometry#getGridToCoordinateSystem}) attribute should also be
	 * {@code null} if the coordinate reference system is {@code null}.
	 * 
	 ...
	 * returns the dimension of the underlying
	 ...
	 * the number of dimensions in the coverage. Grid coverages are typically 2D (<var>x</var>,
	 * <var>y</var>) while other coverages may be 3D (<var>x</var>, <var>y</var>,
	 * <var>z</var>) or 4D (<var>x</var>, <var>y</var>, <var>z</var>,
	 * <var>t</var>). The {@linkplain #getDimension number of dimensions} of
	 * the coverage is the number of entries in the list of dimension names.
	 * <p>
	...
	public InternationalString[] getDimensionNames() {
		if (crs != null) {
			...
			final InternationalString[] names = new InternationalString[cs
					.getDimension()];
			for (int i = 0; i < names.length; i++) {
				names[i] = new SimpleInternationalString(cs.getAxis(i)
						.getName().getCode());
			}
			return names;
		} else {
			final InternationalString[] names = (InternationalString[]) XArray
					.resize(DIMENSION_NAMES, getDimension());
	...
	public boolean[] evaluate(final DirectPosition coord, boolean[] dest)
		...
		try {
			...
			throw new CannotEvaluateException(formatErrorMessage(array),
					exception);
		}
		return dest;
	}
	 ...
	 * Renderable images allow interoperability with <A
	 * HREF="http://java.sun.com/products/java-media/2D/">Java2D</A> for a
	...
	protected class Renderable extends PropertySourceImpl implements
		...
		/**
		 * A coordinate point where to evaluate the function. The point
		 * dimension is equals to the
		 * {@linkplain AbstractCoverage#getDimension coverage's dimension}. The
		 ...
		 */
		protected final GeneralDirectPosition coordinate = new GeneralDirectPosition(
				getDimension());
		...
		public Renderable(final int xAxis, final int yAxis) {
			...
			bounds = new Rectangle2D.Double(envelope.getMinimum(xAxis),
					envelope.getMinimum(yAxis), envelope.getLength(xAxis),
		...
		/**
		 * Gets the minimum <var>X</var> coordinate of the
		 * rendering-independent image data. This is the
		 * {@linkplain AbstractCoverage#getEnvelope coverage's envelope} minimal
		 * value for the {@linkplain #xAxis x axis}.
		 * 
		 * @see AbstractCoverage#getEnvelope
		 * @see AbstractCoverage#getCoordinateReferenceSystem
		 */
		...
		/**
		 * Gets the minimum <var>Y</var> coordinate of the
		 * rendering-independent image data. This is the
		 * {@linkplain AbstractCoverage#getEnvelope coverage's envelope} minimal
		 * value for the {@linkplain #yAxis y axis}.
		 * 
		 * @see AbstractCoverage#getEnvelope
		 * @see AbstractCoverage#getCoordinateReferenceSystem
		 */
		...
		public RenderedImage createRendering(final RenderContext context) {
			...
			if (true) {
				 ...
				 */
				final Rectangle2D bounds = XAffineTransform.transform(
						crsToGrid, (area != null) ? area.getBounds2D()
								: this.bounds, null);
			 ...
			 */
			final Dimension tileSize = ImageUtilities.toTileSize(gridBounds
					.getSize());
			final GridSampleDimension band = GridSampleDimension
					.wrap(getSampleDimension(VISIBLE_BAND));
			final ColorModel colorModel = band.getColorModel(VISIBLE_BAND,
					getNumSampleDimensions());
			final SampleModel sampleModel = colorModel
					.createCompatibleSampleModel(tileSize.width,
							tileSize.height);
			/*
			...
			if ((area == null || area instanceof Rectangle2D)
				...
				image = JAI.create("ImageFunction", new ParameterBlock().add(
						this) // The functional description
						.add(gridBounds.width) // The image width
						.add(gridBounds.height) // The image height
						.add((float) (1 / crsToGrid.getScaleX())) // The X
																	// scale
																	// factor
						.add((float) (1 / crsToGrid.getScaleY())) // The Y
																	// scale
																	// factor
						.add((float) crsToGrid.getTranslateX()) // The X
																// translation
						.add((float) crsToGrid.getTranslateY()), // The Y
																	// translation
						new RenderingHints(JAI.KEY_IMAGE_LAYOUT,
								new ImageLayout().setMinX(gridBounds.x)
										.setMinY(gridBounds.y).setTileWidth(
												tileSize.width).setTileHeight(
												tileSize.height)
				...
				// invocation.
				final GeneralDirectPosition coordinate = new GeneralDirectPosition(
						this.coordinate);
				final TiledImage tiled = new TiledImage(gridBounds.x,
						gridBounds.y, gridBounds.width, gridBounds.height, 0,
						0, sampleModel, colorModel);
				...
				final WritableRectIter iterator = RectIterFactory
						.createWritable(tiled, gridBounds);
							...
							if (!iterator.finishedPixels()) {
								...
								do {
									...
									if (area == null || area.contains(point2D)) {
										...
										iterator.setPixel(evaluate(coordinate,
												samples));
									} else {
			 ...
			 * information for constructing a GridCoverage from this image
			 * later.
			 */
			try {
		 ...
		 * 
		 * @param gridBounds
		 *            The two-dimensional destination rectangle.
		 * @param hints
		 *            The rendering hints, or {@code null} if none.
		...
		protected RenderContext createRenderContext(
			...
			if (crs != null) {
				...
				final AxisDirection[] normalized = (AxisDirection[]) axis
						.clone();
				if (false) {
				...
				normalized[1] = normalized[1].opposite(); // Image's Y axis is
															// downward.
				matrix = new GeneralMatrix(srcEnvelope, axis, dstEnvelope,
						normalized);
			} else {
		 ...
		 * coordinates. This method is automatically invoked at rendering time
		 * for populating an image tile, providing that the rendered image is
		 * created using the "{@link ImageFunctionDescriptor ImageFunction}"
		 * operator and the image type is not {@code double}. The default
		 * implementation invokes
		 * {@link AbstractCoverage#evaluate(DirectPosition,float[])}
		 * recursively.
		 */
		public void getElements(final float startX, final float startY,
				final float deltaX, final float deltaY, final int countX,
				final int countY, final int element, final float[] real,
				final float[] imag) {
			int index = 0;
			float[] buffer = null;
			// Clones the coordinate point in order to allow multi-thread
		 ...
		 * for populating an image tile, providing that the rendered image is
		 * created using the "{@link ImageFunctionDescriptor ImageFunction}"
		 * operator and the image type is {@code double}. The default
		 * implementation invokes
		 * {@link AbstractCoverage#evaluate(DirectPosition,double[])}
		 * recursively.
		 */
		public void getElements(final double startX, final double startY,
				final double deltaX, final double deltaY, final int countX,
				final int countY, final int element, final double[] real,
				final double[] imag) {
			int index = 0;
			double[] buffer = null;
			// Clones the coordinate point in order to allow multi-thread
	...
	public void dispose() {
		...
		// Note: implementing this method in GridCoverage is tricky. We must
		// ensure that:
		// 1) The PlanarImage is not used by somebody else (i.e. is not a user
		// supplied
	...
	public Set evaluateInverse(Record arg0) {
		throw new UnsupportedOperationException(
				"This method is currently not impemented. It will be implemented by next version of coverage.");
	}

	public GeometryValuePair find(DirectPosition arg0) {
		throw new UnsupportedOperationException(
				"This method is currently not impemented. It will be implemented by next version of coverage.");
	}

	public List find(DirectPosition arg0, int arg1) {
		throw new UnsupportedOperationException(
				"This method is currently not impemented. It will be implemented by next version of coverage.");
	}
