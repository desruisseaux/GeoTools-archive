...
final class Resampler2D extends GridCoverage2D {
    ...
    /**
     * Disables the native acceleration for the "Affine" operation. In JAI 1.1.2, the "Affine"
     ...
     * @param  hints The rendering hints. This is usually provided by {@link AbstractProcessor}.
            ...
            throws FactoryException, TransformException
    {
        ...
        if (sameCRS) {
            ...
            if (!targetGG.isDefined(GridGeometry2D.GRID_TO_CRS)) {
                step1    = sourceGG.getGridToCoordinateSystem(); // Really sourceGG, not targetGG
            ...
            if (targetGG == null) {
                ...
                if (!targetGG.isDefined(GridGeometry2D.GRID_RANGE)) {
                    ...
                    gridRange = CRSUtilities.transform(step1.inverse(), targetEnvelope);
    ...
    private static boolean equivalent(final GridGeometry2D geom1, final GridGeometry2D geom2) {
        ...
        if (geom1.isDefined(GridGeometry2D.GRID_RANGE) && geom2.isDefined(GridGeometry2D.GRID_RANGE)) {
            if (!geom1.getGridRange().equals(geom2.getGridRange())) {
                return false;
            }
        }
        if (geom1.isDefined(GridGeometry2D.GRID_TO_CRS) && geom2.isDefined(GridGeometry2D.GRID_TO_CRS)) {
