...
public class CustomGlyphRenderer implements GlyphRenderer {
	...
	public CustomGlyphRenderer() {
		...
		list.addProperty("radius", Expression.class, factory
				.createLiteralExpression(50));
		list.addProperty("circle color", Expression.class, factory
				.createLiteralExpression("#000066"));
		list.addProperty("bar height", Expression.class, factory
				.createLiteralExpression(150));
		list.addProperty("bar color", Expression.class, factory
				.createLiteralExpression("#000000"));
		list.addProperty("bar uncertainty", Expression.class, factory
				.createLiteralExpression(50));
		list.addProperty("bar uncertainty width", Expression.class, factory
				.createLiteralExpression(5));
		list.addProperty("bar uncertainty color", Expression.class, factory
				.createLiteralExpression("#999999"));
		list.addProperty("pointer length", Expression.class, factory
				.createLiteralExpression(100));
		list.addProperty("pointer color", Expression.class, factory
				.createLiteralExpression("#FF0000"));
		list.addProperty("pointer direction", Expression.class, factory
				.createLiteralExpression(21));
		list.addProperty("wedge width", Expression.class, factory
				.createLiteralExpression(25));
		list.addProperty("wedge color", Expression.class, factory
				.createLiteralExpression("#9999FF"));
	}
	...
	public String getGlyphName() {
		return "exploded clock"; // I think Alan called it this once, so here
		// it sticks.
	}
	...
	// Is this really necessary, since the values in the properties list can be
	// set?
	...
	public BufferedImage render(Graphic graphic, ExternalGraphic eg,
		...
		while (it.hasNext()) {
			...
			if (list.hasProperty(nextName)) {
				...
				// DO I WANT TO THROW AN EXCEPTION OR ADD THE NEW PROPERTY TO
				// THE LIST OR DO NOTHING?
		...
		// calculate maximum value of barHeight + barUncertainty & use that
		// instead of "barHeight + barUncertainty"
		if (!maxFound) {
			maxFound = true;
			...
			while (features.hasNext()) {
				next = features.next();
				tempExp = (Expression) list.getPropertyValue("bar height");
				temp1 = 0;
				if (tempExp != null) {
		...
		imageHeight = Math.max(radius * 2, Math.max(radius + pointerLength,
		...
		imageGraphic.fillOval(circleCenterX - radius, circleCenterY - radius,
				radius * 2, radius * 2);
		imageGraphic.setColor(wedgeColor);
		imageGraphic.fillArc(circleCenterX - radius, circleCenterY - radius,
				radius * 2, radius * 2, calculateWedgeAngle(pointerDirection,
						wedgeWidth), wedgeWidth * 2);
		imageGraphic.setColor(barUncColor);
		imageGraphic.fillRect(circleCenterX - barUncWidth, circleCenterY
				- barHeight - barUncertainty, barUncWidth * 2,
				barUncertainty * 2);
		// pointer
		int[] endPoint = calculateEndOfPointer(circleCenterX, circleCenterY,
				pointerLength, pointerDirection);
		imageGraphic.setStroke(new java.awt.BasicStroke(3));
		imageGraphic.setColor(pointerColor);
		imageGraphic.draw(new java.awt.geom.Line2D.Double(circleCenterX,
				circleCenterY, endPoint[0], endPoint[1]));
		// bar
		imageGraphic.setStroke(new java.awt.BasicStroke(3));
		imageGraphic.setColor(barColor);
		imageGraphic.draw(new java.awt.geom.Line2D.Double(circleCenterX,
				circleCenterY, circleCenterX, circleCenterY - barHeight));
	...
	private int[] calculateEndOfPointer(int circleCenterX, int circleCenterY,
			int pointerLength, int pointerDirection) {
		int x = circleCenterX
				+ (int) Math.round(pointerLength
