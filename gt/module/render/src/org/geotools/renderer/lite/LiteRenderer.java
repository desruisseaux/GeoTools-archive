...
public class LiteRenderer implements Renderer, Renderer2D {
	 ...
	 * 
	 * @param context
	 *            Contains pointers to layers, bounding box, and style required
	 *            for rendering.
	 */
	 ...
	 * 
	 * @param g
	 *            A graphics object for future rendering to be sent to. Note:
	 *            must be an instance of lite renderer.
	 * @param bounds
	 *            The size of the output area, required so that scale can be
	 *            calculated.
	...
	public void paint(Graphics2D graphics, Rectangle paintArea,
		...
		try {
			setScaleDenominator(calculateScale(envelope, context
					.getCoordinateReferenceSystem(), paintArea.width,
					paintArea.height, 90));// 90 = OGC standard DPI (see SLD
			// spec page 37)
		...
		for (int i = 0; i < layers.length; i++) {
			...
			try {
				...
				// and process them
				processStylers(graphics, results, currLayer.getStyle()
						.getFeatureTypeStyles(), at, context
						.getCoordinateReferenceSystem(), currLayer
						.getFeatureSource().getSchema().getDefaultGeometry()
						.getCoordinateSystem()); // src CRS
		...
		if (error > 0) {
			LOGGER
					.warning("Number of Errors during paint(Graphics2D, AffineTransform) = "
							+ error);
		}
	}
	...
	public static double calculateScale(Envelope envelope,
			...
			int imageWidth, int imageHeight, double DPI) throws Exception {
		double diagonalGroundDistance = JTS.orthodromicDistance(new Coordinate(
				envelope.getMinX(), envelope.getMinY()), new Coordinate(
	...
	FeatureResults queryLayer(MapLayer currLayer, Envelope envelope,
		...
		if (optimizedDataLoadingEnabled) {
			...
			try {
				...
				// each
				// geometric
				// attribute used during the rendering as the feature may have
				// more
				// than one
				// and the styles could use non default geometric ones
				CoordinateReferenceSystem sourceCrs = currLayer
						.getFeatureSource().getSchema().getDefaultGeometry()
						.getCoordinateSystem();
				...
				if (sourceCrs != null && !sourceCrs.equals(destinationCrs)) {
					...
					transform = operationFactory.createOperation(
							destinationCrs, sourceCrs).getMathTransform();
					if (transform != null && !transform.isIdentity()) {
						// Envelope eee= JTS.transform(envelope, transform);//
						// this is the old way
						// 10 = make 10 points on each side of the bbox &
						// transform the polygon
						envelope = org.geotools.geometry.JTS.transform(
								envelope, transform, 10); // this will usually
						// be a "bigger"
						// bbox
					} else
		...
		((DefaultQuery) query).setCoordinateSystem(currLayer.getFeatureSource()
		...
		if (memoryPreloadingEnabled) {
			...
			if (indexedFeatureResults == null) {
				indexedFeatureResults = new IndexedFeatureResults(featureSource
						.getFeatures(query));
			}
	...
	private String[] findStyleAttributes(MapLayer layer, FeatureType schema) {
		...
		for (int i = 0; i < attTypes.length; i++) {
			...
			if ((attTypes[i] instanceof GeometryAttributeType || attTypes[i]
					.getName().equalsIgnoreCase("grid"))
		...
		for (int j = 0; j < attributes.length; j++) {
			...
			if (attType == null)
				throw new IllegalFilterException("Could not find '"
						+ attributes[j] + "' in the FeatureType ("
						+ schema.getTypeName() + ")");
	 ...
	 * style object may cause some features to be rendered multiple times or not
	 * at all.
	 * </p>
	 * 
	 * @param features
	 *            the feature collection to be rendered
	 * @param map
	 *            Controls the full extent of the input space. Used in the
	 *            calculation of scale.
	 * @param s
	 *            A style object. Contains a set of FeatureTypeStylers that are
	...
	public void render(FeatureCollection features, Envelope map, Style s) {
		...
		// extract the feature type stylers from the style object and process
		// them
		FeatureTypeStyle[] featureStylers = s.getFeatureTypeStyles();

		try {
			processStylers(outputGraphics, DataUtilities.results(features),
					featureStylers, at, null, null);
		} catch (IOException ioe) {
			fireErrorEvent(new Exception("I/O error while rendering the layer",
					ioe));
		} catch (IllegalAttributeException iae) {
			fireErrorEvent(new Exception(
					"Illegal attribute exception while rendering the layer",
					iae));
		}
		...
		for (int i = 0; i < featureStylers.length; i++) {
			...
			if ((typeName != null)
				...
				if ((ruleList.size() == 0) && (elseRuleList.size() == 0))
					return; // DJB: optimization - nothing to render, dont query
				// layer!!
				...
				CoordinateReferenceSystem rCS = reader.getFeatureType()
				...
				// same CRS object
				{
					...
					// really happen)
					{
						...
						if ((rCS == null) || (!rCS.equals(sourceCrs))) {
							...
							try {
								reader = new ForceCoordinateSystemFeatureReader(
										reader, sourceCrs);
							} catch (Exception ee) {
								ee.printStackTrace(); // do nothing but warn
								// user
							}
						}
					}
	...
	private LiteShape2 getTransformedShape(Geometry g, MathTransform2D transform)
		...
		LiteShape2 shape = new LiteShape2(g, transform,
				getDecimator(transform), false);
		return shape;
	}
	...
	private MathTransform2D getMathTransform(
		...
		MathTransform2D transform = (MathTransform2D) transformMap
				.get(sourceCrs);
	 ...
	 * is ignored and the renderer tries to depict the non geophysics version of
	 * the grid coverage on the device, and falls back on the geophysics one if
	 * the former fails
	 * 
	 * @param graphics
	 *            DOCUMENT ME!
	...
	private void renderRaster(Graphics2D graphics, Feature feature,
			...
			CoordinateReferenceSystem destinationCRS) {
		LOGGER.fine("rendering Raster for feature " + feature.toString()
				+ " - " + feature.getAttribute("grid"));
		...
		try {
			...
			gcr.paint(graphics, (GridCoverage2D) feature.getAttribute("grid"),
					symbolizer);
	...
	// ---------------------------------------------------------------------------------------------
	// Rescued From previous version of LiteRenderer
	// ---------------------------------------------------------------------------------------------
	 ...
	 * @param fill
	 *            how to fill the feature
	 * @param feature
	 *            the feature to be filled
	 */
	public void applyFill(Graphics2D graphic, Fill fill, Feature feature) {
		...
		graphic.setColor(Color.decode((String) fill.getColor()
				.getValue(feature)));
	 ...
	 *            graphics context used for drawing
	 * @param gr
	 *            the graphic specifiying the texture
	 * @param feature
	...
	public void setTexture(Graphics2D graphic, Graphic gr, Feature feature) {
		...
		/*
		 * This is needed because the image must be a fixed size in pixels but
		 * when the image is used as the fill it is transformed by the current
		 * transform. However this causes problems as the image size can become
		 * very small e.g. 1 or 2 pixels when the drawScale is large, this makes
	 ...
	 * 
	 * @param graphic
	 *            The graphic object containing the mark list
	 * @param feature
	 *            The feature that will be depicted with the mark
	 * 
	 ...
	 * 
	 * @param graphic
	 *            the graphic associated to the mark
	 * @param point
	 *            mark location
	 * @param mark
	 *            the mark
	 ...
	 *            DOCUMENT ME!
	 * @param feature
	 *            the feature associated to the mark
	 */
	 ...
	 *            the graphic associated to the mark
	 * @param tx
	 *            mark location, x coordinate
	 * @param ty
	 *            mark location, y coordinate
	 * @param mark
	 *            the mark
	 * @param size
	 *            mark size
	...
	private void fillDrawMark(Graphics2D graphic, double tx, double ty,
			Mark mark, int size, double rotation, int xOffset, int yOffset,
			Feature feature) {
	...
	private void applyStroke(Graphics2D graphic, Stroke stroke, Feature feature) {
		...
		if ((dashes != null) && (dashes.length > 0)) {
			...
			stroke2d = new BasicStroke(width / (float) scale, capCode,
					joinCode, (float) 1, dashes, dashOffset / (float) scale);
		} else {
			if (width <= 1.0) {
				width = 0;
			}
		...
		Color color = Color
				.decode((String) stroke.getColor().getValue(feature));
