...
public class GT2Eclipse {
	...
	public static void generatebuildlist(String path) {
		try {
			File srcdir = new File(path, "geotools-src");
	...
	public static Set targets(String path) {
		try {
			...
			if (buildlist == null) {
				buildReport = new BufferedReader(new FileReader(new File(path,
						"buildReport.txt")));
			} else {
				buildReport = new BufferedReader(new FileReader(new File(path,
						buildlist)));
			...
			e.printStackTrace();
			System.err
					.println("Please run maven.build from your geotools2 directory."
	...
	static public Set dependencies(String path, Set targets) {
		...
		// read file
		/*
		 * for( Iterator i=targets.iterator(); i.hasNext();){ depends.addAll(
		 * process( path, "geotools-src/"+i.next()+"/project.xml" )); }
		 */
	...
	public static void main(String args[]) {
		...
		if (args.length == 0) {
			...
			System.out.println("use: java GT2Eclipse [OPTION] [geotools2directory]");
			System.out.println();
