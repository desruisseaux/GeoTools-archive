...
if [ ! -e "$REPO_LOCAL" ]; then
  echo
  echo "Please edit this script and redefine the REPO_LOCAL variable"
  exit
fi
...
#group, artifact, version
{
  if [ -e "$REPO_LOCAL/$1/$2/$3/$2-$3.pom" ]; then
    cd "$REPO_LOCAL/$1/$2/$3"
    DEPLOY_CMD="mvn deploy:deploy-file -Durl=$REPO_REMOTE -DrepositoryId=$REPO_ID -DpomFile=$2-$3.pom -Dfile=$2-$3.jar"
    ...
    echo "MISSING: $REPO_LOCAL/$1/$2/$3/$2-$3.pom"
  fi
}
...
deploy_file com/vividsolutions jts 1.7
...
deploy_file postgresql postgresql 8.0-312.jdbc3
...
#deploy_file velocity velocity 1.4
#deploy_file org/postgis postgis-driver 1.0
deploy_file postgresql postgresql 8.1-404.jdbc3
#deploy_file mysql mysql-connector-java 3.0.10
#deploy_file hsqldb hsqldb 1.8.0.1
#deploy_file org/openplans spatialdb 0.1
