...
public class ParserTest extends FilterTestSupport {
	...
	public void setUp() {
		...
		SimpleFeatureType superType = testSchema;
		List<AttributeType> types = Arrays
				.asList(new AttributeType[] { typeFactory.createType(
						"testZeroDouble", Double.class) });
		
		testSchema = typeFactory.createFeatureType(new QName("test"), types, null, null, superType, false);
	...
	/*
	 * 18 and 19 have multiple filters, and only last one will print, but
	 * logicSAXParser was messing up with multiple filters, and I used these
	 * tests to fix it. To make these effective the parser test should be able
	 * to get multiple filters. And shouldn't we also be checking the filters
	 * generated programmatically, so they fail if things mess up? I don't have
	 * time right now, but maybe some time soon...cholmes
	 */
