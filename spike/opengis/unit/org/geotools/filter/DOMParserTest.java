...
public class DOMParserTest extends FilterTestSupport {
	 ...
	 * 
	 * @param args
	 *            the passed in arguments (not used).
	 */
	...
	public void setUp()  {
		...
		/*
		 * FeatureTypeBuilder feaTypeFactory =
		 * FeatureTypeFactory.createTemplate(testSchema); AttributeType
		 * doubleAttribute2 = attFactory.newAttributeType("testZeroDouble",
		 * Double.class); feaTypeFactory.addType(doubleAttribute2); testSchema
		 * =feaTypeFactory.getFeatureType();
		 */

		AttributeType[] types = { typeFactory.createType("testZeroDouble",
				Double.class) };

		SimpleFeatureType baseType = testSchema;
		QName typeName = baseType.getName();

		testSchema = typeFactory.createFeatureType(typeName, Arrays
				.asList(types), null, null, baseType, false);
	...
	public Filter parseDocument(String uri) throws Exception {
		...
		Document dom = db.parse(TestData.getResource(this, uri)
				.toExternalForm());
