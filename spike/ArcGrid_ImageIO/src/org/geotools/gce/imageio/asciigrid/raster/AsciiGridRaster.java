...
public abstract class AsciiGridRaster {
	...
	private final int[] mutex = new int[1];
	...
	public WritableRaster readRaster(ImageReadParam param) throws IOException {
		...
		Long key;
		Long val;
		int tileDim=tileHeight * tileWidth;
		...
		boolean sampleToUse = (!((tempCol < srcRegionXOffset || tempCol >= srcRegionXOffset
				+ srcRegionWidth)))
				&& ((!doSubsampling) || (doSubsampling && (((tempRow)
				...
				if ((prevCh == 32) // ' '
					...
					if (((decimalPoint == 1) && (decimalDigits == 0))
						...
						throw new NumberFormatException(new StringBuffer(
								...
								.append(decimalPoint).append(
										"\nDecimalDigits = ").append(
										decimalDigits).append("\nE Symbols = ")
								.append(expChar).append(
										"\nDigits after E Symbol = ").append(
										expDigits).append("\n").toString());
					}

					// //
					//
					// Does subsampling/subregion allow to add this value?
					//
					// //

					// //
					//
					// Are we inside the source region?
					//
					// //
					if (sampleToUse)
					...
					// the following values indicate where this value is
					// positioned in the original matrix
					// which is nCols*nRows
					tempCol = samplesCounted % nCols;
					tempRow = samplesCounted / nCols;
					//updating the 
					sampleToUse = (!((tempCol < srcRegionXOffset || tempCol >= srcRegionXOffset
							+ srcRegionWidth)))
							&& ((!doSubsampling) || (doSubsampling && (((tempRow)
									% ySubsamplingFactor == 0) && ((tempCol)
									% xSubsamplingFactor == 0))));
